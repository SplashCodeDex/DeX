...
namespace DeXShareTarget.Endpoints
{
    public static partial class LocalSendEndpoints
    {
        ...
        {
            app.MapPost("/api/dex/clipboard", async (HttpRequest request) => await ClipboardService.HandlePhonePushAsync(request));
            ...
            app.MapPost("/local/clipboard-sync", (HttpRequest request) =>
            {
                ...
                {
                    ClipboardService.IsSyncEnabled = value;
                    return Results.Ok();
                }
                return Results.BadRequest();
            });

            app.MapGet("/local/clipboard-state", () => ClipboardService.GetState());
            ...
            app.MapPost("/local/clipboard-push", async (HttpRequest request) => await ClipboardService.HandleLocalPushAsync(request));
