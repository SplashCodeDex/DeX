...
namespace DeXShareTarget.Services
{
    ...
    /// redirect routed through the existing Kestrel localhost API (port 48425) so no
    /// second TcpListener is needed. The verified email becomes this PC's identity,
    ...
    {
        // Routed through Kestrel's already-proven localhost listener — no separate port.
        private const string RedirectUri = "http://127.0.0.1:48425/local/oauth/callback";
    ...
    /// <summary>
    /// Called by the Kestrel endpoint when the browser redirect arrives.
    /// Completes the matching pending sign-in flow, or is a no-op if no flow
    /// is waiting on this state (expired, already completed, or not ours).
    /// </summary>
    public static bool HandleCallback(string state, string code, out string? error)
    {
        error = null;
        if (_pending.TryRemove(state, out var tcs))
        {
            ...
            return tcs.TrySetResult(code);
        }
        error = "No pending sign-in for this state (may have expired).";
        return false;
    }
    ...
    public static async Task<GoogleProfile?> SignInAsync(CancellationToken ct = default)
    {
        ...
        // Register a TCS then open the browser — Kestrel resolves it via HandleCallback.
        ...
        // Wait for the redirect (or timeout/cancellation).
        using var timeoutCts = CancellationTokenSource.CreateLinkedTokenSource(ct);
        timeoutCts.CancelAfter(TimeSpan.FromSeconds(230));
        using var reg = timeoutCts.Token.Register(() =>
        {
            if (_pending.TryRemove(state, out var expired))
                expired.TrySetResult(null);
        });
