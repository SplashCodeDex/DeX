...
## [Unreleased]
### Fixed
- **[fix] Android SAF storage sandbox lifecycle, MediaStore fallback & name sanitization (`SafStorage.kt`, `SafStorageTest.kt`)**:
  - **MediaStore Sandbox IS_PENDING Lifecycle & Visibility**: In `SafStorage.saveUrisToSandbox`, invoked `finishMediaStoreUri(context, mediaUri)` upon successful file copying. Previously, `saveUrisToSandbox` created MediaStore rows with `IS_PENDING = 1` but never cleared the flag, leaving files permanently hidden from other apps, media scanners, and user file explorers. In addition, wrapped output stream writing to delete incomplete/corrupt MediaStore entries via `deleteUri(context, mediaUri)` when stream writing throws or fails.
  - **False Success Prevention on Output Stream Failures**: In both `writeFile` and `saveUrisToSandbox`, checked whether `openOutputStream` returned null instead of relying on discarded `?.use` expressions which previously evaluated to null and fell through to `true`. When output streams cannot be opened, `writeFile` immediately cleans up the created SAF document via `deleteUri(context, doc)` and returns `false`.
  - **SAF-to-MediaStore Transparent Fallback**: When `getDownloadsDexUri` returns a granted tree URI but writing fails (e.g. revoked permissions, full disk, or invalid tree location), `saveUrisToSandbox` now transparently falls back to `createMediaStoreUri` with a fresh input stream rather than reporting 0 saved files and failing the batch.
  - **Per-Item Loop Isolation & Traversal Sanitization**: Replaced the fragile outer `try ... catch` with per-URI exception handling so that a `SecurityException` or unreadable stream on one shared URI does not abort subsequent valid files in the batch. Introduced `sanitizeFileName` to strip directory traversal sequences (`..`), separators (`/`, `\`), and illegal characters before passing names to SAF or MediaStore.
  - **Regression Verification**: Added unit tests in `SafStorageTest` verifying `IS_PENDING` clearing, null stream rejection, loop isolation across failing URIs, SAF-to-MediaStore fallback, and filename path traversal sanitization.
- **[fix] Android upload worker cancellation lifecycle, error reporting & activeWorkId cleanup (`UploadWorker.kt`, `PunchSendWorker.kt`, `UploadWorkerTest.kt`)**:
  - **Notification Cancellation Funnel & UploadState Propagation**: When an active upload was cancelled via Android WorkManager notification action ("Cancel") or timeout, `UploadWorker.doWork` previously caught `CancellationException` and re-threw it without updating `client.updateUploadState` or clearing `client.activeWorkId`. As a result, `client.uploadState.value.isUploading` remained permanently `true`, locking the UI in a perpetual "sending" state. Added `catch (e: CancellationException)` to immediately transition `UploadState(isUploading = false, error = "Upload cancelled")`, clear `client.activeWorkId = null`, and notify the UI before re-throwing.
  - **Terminal HTTP Status & Rejection Error Reporting**: When `prepareUpload` returned `null` (e.g. peer rejected with 401/403 unauthorized, 507 insufficient disk space, 400 bad request, or transport retries exhausted), `UploadWorker` previously returned `Result.failure()` without updating `client.uploadState.error` (which had been cleared by `resetUploadState`). Added `determineTerminalError(httpStatus)` to map HTTP response codes to human-readable error reasons, update `client.updateUploadState(UploadState(isUploading = false, error = errorMsg))`, log failed files into `TransferHistory`, and clear `client.activeWorkId = null`.
  - **NAT Hole-Punch Send Worker Cancellation & Exception Guard**: In `PunchSendWorker.kt`, wrapped `punchSession.sendTo` and `relayViaPc` in `try ... catch (CancellationException)` and `catch (e: Exception)` to capture cancellations and failures, update `UploadState(isUploading = false, error = ...)`, and clear `client.activeWorkId = null`.
  - **Regression Verification**: Added unit tests in `UploadWorkerTest` demonstrating both rejection error reporting and cancellation lifecycle handling without state hangs.
- **[fix] Desktop file send active session cancellation funnel & false success prevention (`ClientEngine.kt`, `DesktopFileSendService.kt`, `TransferOverlayBridgeTest.kt`, `DesktopTransferOwnershipTest.kt`, `ClientEngineTransferContractTest.kt`)**:
  - **Centralized Cancellation Funnel & Active Session Binding**: In `ClientEngine.kt`, introduced a thread-safe listener registry (`registerCancelHandler` and `unregisterCancelHandler`) invoked whenever `cancelUpload()` is executed. In `DesktopFileSendService`, bound an `init` hook registering `cancelActiveSession()`. Previously, `clientEngine.cancelUpload()` only cancelled Android WorkManager tasks via `activeWorkId`, leaving `DesktopFileSendService.sessionJob` uncancelled. Outbound file uploads continued transferring bytes in the background despite the desktop user clicking "Cancel" on the overlay banner.
  - **False Success State & Post-Cancellation Guard**: In `DesktopFileSendService.kt`, guarded `finishUpload` calls in both `executeSessionAttempt` and `sendViaWanRelay` behind `if (sessionCancelled) return ...`. Previously, background coroutines that continued transferring or concluded after cancellation invoked `clientEngine.finishUpload`, causing `UploadState.isSuccess` to flip to `true` and prompting `TransferOverlayBridge` to display false-positive "Sent 'files' successfully" notifications for aborted transfers.
  - **Regression Verification**: Added unit tests in `DesktopTransferOwnershipTest` proving that calling `clientEngine.cancelUpload()` cancels the active `DesktopFileSendService` session and prevents `finishUpload` from firing; in `ClientEngineTransferContractTest` verifying listener invocation; and in `TransferOverlayBridgeTest` proving that banner cancel actions invoke `clientEngine.cancelUpload()` and dismiss the active banner.
- **[fix] Desktop transfer checkpoint nested parent directory creation & commit resilience (`TransferCheckpointRegistry.kt`, `TransferCheckpointRegistryTest.kt`)**:
  - **Nested Parent Directory Creation on Commit**: In `TransferCheckpointRegistry.kt`, added explicit parent directory creation (`free.parentFile?.mkdirs()` and `destFile.parentFile?.mkdirs()`) before performing `part.renameTo(free)` and inside `createUniquePlaceholder`. Previously, when an incoming transfer contained a relative path with uncreated subfolders (e.g. `reports/2026/quarterly.pdf`), `destFile` pointed into a non-existent directory. Both `part.renameTo(free)` and `createUniquePlaceholder` failed, returning `null` from `commitPartFile` and causing `ShareRoutes` to answer `500 InternalServerError` despite the file having arrived completely and cleanly in staging.
  - **Staging Parent Directory Defense-in-Depth**: In `getOrCreatePartFile`, invoked `parentDir.mkdirs()` to guarantee the parent staging directory exists before creating `.part` checkpoint files on disk.
  - **Regression Verification**: Added an empirical unit test in `TransferCheckpointRegistryTest` proving that `commitPartFile` to deeply nested, non-existent parent directories succeeds, automatically creates all parent directory levels, and cleans up the staging `.part` file upon successful commit.
- **[fix] Desktop pull transfer cancellation routing, progress integrity & false receive prevention (`FileExplorerService.kt`, `TransferOverlayBridge.kt`, `WebSocketRoutes.kt`, `FileExplorerServiceTest.kt`, `TransferOverlayBridgeTest.kt`)**:
  - **Pull Progress State Enrichment & Targeted Cancellation**: Added `isCancelled: Boolean = false`, `isFailed: Boolean = false`, and `fingerprint: String = ""` to `PullProgressState` in `FileExplorerService.kt`. `pullFiles` now populates `fingerprint` directly on the progress state. In `cancelPull`, resolved `fingerprint` fallback from `_pullProgress.value.fingerprint` or active connected WebSocket sessions when blank, and resolved `requestId` fallback from active pull progress. `cancelPull` dispatches `MessageTypes.PULL_CANCEL` and transitions the state to `isPulling = false, isDone = false, isCancelled = true`, fixing a critical bug where cancellation previously set `isDone = true`.
  - **Desktop Overlay Action Callback & False Receive Prevention**: In `TransferOverlayBridge.kt`, updated banner cancellation action callback to pass `state.fingerprint` and `state.requestId` to `fileExplorerService.cancelPull`. In the completion handler, gated the AirDrop receive alert (`overlayManager.showAlert`) behind `state.isDone && !state.isCancelled && !state.isFailed`, completely eliminating false-positive "Received 'file' - Saved to Downloads/DeX" success alerts when a user or peer aborts or fails a pull operation.
  - **Stale WebSocket Progress Isolation & State Mapping**: In `WebSocketRoutes.kt`, added request ID validation to `MessageTypes.PULL_PROGRESS` handling (`current.isPulling && current.requestId.isNotBlank() && current.requestId != reqId`), discarding out-of-order or late progress frames from superseded pull requests. Correctly mapped incoming terminal states (`STATE_DONE`, `STATE_CANCELLED`, `STATE_FAILED`) onto `isDone`, `isCancelled`, and `isFailed` while preserving device fingerprint.
  - **Regression Verification**: Added unit tests in `FileExplorerServiceTest` verifying that `pullFiles` registers fingerprint and request metadata, that `cancelPull` resolves omitted fingerprints and broadcasts `PULL_CANCEL`, and that cancelled transfers set `isDone = false` and `isCancelled = true`. Added tests in `TransferOverlayBridgeTest` verifying that banner cancellation forwards the device fingerprint and dismisses without spawning receive alert cards.
- **[fix] Pairing PIN brute-force lockout, attempt threshold & state invalidation (`PairingEngine.kt`, `WebSocketRoutes.kt`, `PinPairingPanel.kt`, `PairingEngineTest.kt`)**:
  - **Consecutive Failed Attempt Rate-Limiting & Permanent Lockout**: In `PairingEngine.kt`, added `failedAttempts: Int = 0` to `PairingState.PinPhase` and introduced `MAX_PIN_ATTEMPTS = 3`. In `verifyInboundPin`, incremented the attempt counter on mismatch when matching the active pending peer's fingerprint. Once failed attempts reach `MAX_PIN_ATTEMPTS`, the engine immediately cancels `expiryJob` and transitions to `PairingState.Error("Too many failed PIN attempts")`, permanently invalidating the pairing offer so that subsequent attempts (even if submitting the correct PIN) are rejected. An unrelated peer's fingerprint cannot increment the pending peer's failed attempts, preventing denial-of-service lockout attacks.
  - **Dynamic Error Recovery & Telemetry Reset**: `handlePinDigitEntered` now clears `isError` when digits are entered (`digitCount > 0`) during an active `PinPhase`, allowing legitimate users to receive immediate feedback when typing a retry after a typo. Fixed `armExpiry` to ensure unresolved offers with error feedback still honor the 60s TTL deadline and transition cleanly to `"Pairing timed out"`.
  - **Server WebSocket Unproven Assertion & Lockout Routing**: In `WebSocketRoutes.kt`, updated `MessageTypes.PAIR_RESPONSE` handling so that unproven assertions (`accepted: true` without PIN) do not abort the pairing session, preserving the desktop user's ability to manually accept via the panel. Incorrect PIN submissions update the attempt counter and trigger lockout without premature rejection bypasses, while explicit user declines (`!accepted`) route to clean cancellation.
  - **Desktop UI Failure Subtitle Integration**: In `PinPairingPanel.kt`, updated `PairingState.Error` rendering to use `s.message.ifBlank { "Pairing failed" }`, surfacing "Too many failed PIN attempts" and other failure reasons directly to the user.
  - **Regression & Lockout Verification**: Added unit tests in `PairingEngineTest` verifying attempt incrementing, telemetry error reset, cross-fingerprint isolation, and permanent session lockout after 3 consecutive failed PIN attempts.
- **[fix] Cross-platform clipboard echo-guard priming, payload threshold & ADB broadcast bounds (`clipboardRoutes.kt`, `DesktopClipboardSync.kt`, `ClipboardReceiver.kt`, `ClipboardRoutesTest.kt`)**:
  - **Desktop Route Echo-Guard Priming & Payload Bounds**: In `clipboardRoutes.kt`, routed inbound `/api/dex/clipboard` requests through `ClipboardSyncState.applyRemoteText(text)` (invoking `ClipboardSyncUseCase.applyRemoteClipboard`) rather than directly setting AWT system clipboard. Direct AWT writes previously bypassed the echo guard, leaving `lastReceivedHash` unset and causing AWT change listeners to bounce the exact same text back to the sender in an infinite sync loop. In addition, enforced `MAX_CLIPBOARD_TEXT_BYTES` (1MB) threshold on incoming text length and `Content-Length`, rejecting oversized payloads with `413 Payload Too Large` to prevent memory exhaustion.
  - **Android ADB Broadcast Echo Prevention & Safe Decoding**: In `ClipboardReceiver.kt`, set `ClipboardSyncState.lastIncoming = text` before committing clips to Android's `ClipboardManager`. Previously, receiving clipboard broadcasts via ADB (`SET_CLIPBOARD`) left `lastIncoming` unset, prompting `ClipboardSyncManager`'s listener to interpret the incoming clip as a fresh local phone copy and push it right back to the PC. Added safe Base64 decode error handling to guard against malformed intent extras.
  - **Desktop ADB Broadcast Command-Line Bound**: In `DesktopClipboardSender.kt`, introduced `MAX_ADB_CLIPBOARD_BYTES` (16KB) upper bound for the ADB broadcast fallback. Text payloads exceeding this threshold avoid `am broadcast -e` command line parameter expansion, preventing Win32 `CreateProcess error=206` (The filename or extension is too long) crashes.
  - **Regression Verification**: Added unit tests in `ClipboardRoutesTest` proving that `/api/dex/clipboard` primes the echo guard to suppress re-broadcast and that payloads exceeding 1MB are rejected with `413 Payload Too Large`.
- **[fix] Desktop multi-file concurrent upload cancellation & transfer session synchronization (`ShareRoutes.kt`, `TransferUseCase.kt`, `ShareRoutesTest.kt`, `TransferUseCaseTest.kt`)**:
  - **Multi-File Concurrent Upload Cancellation**: Fixed a race condition in `ShareRoutes.kt` where `activeUploadJobs[sessionId]` stored only a single `Job`. In multi-file concurrent transfers, newly arriving uploads overwrote previously registered upload jobs for the same session ID. As a result, issuing `/api/localsend/v2/cancel` only cancelled the most recently registered upload, allowing earlier in-flight uploads to continue writing, commit files to disk, and falsely report success. Implemented session coordinator jobs (`SupervisorJob`) with `activeUploadCounts` tracking, binding all concurrent child upload coroutines so that `/cancel` or session expiration immediately aborts all active uploads in the session.
  - **Transfer Session Registry Concurrency & State Preservation**: In `TransferUseCase.kt`, synchronized `registerSession` under `lock`, resolving unsynchronized map writes that raced against `reportProgress`, `markComplete`, and `removeSession`. Furthermore, `registerSession` preserves existing in-flight session progress (`filesReceived`, `bytesReceived`, `isComplete`, `speedBps`, `etaSeconds`) when called for an existing session (e.g. from `TransferStateMonitor.updateIncomingProgress`), preventing progress metrics from being reset to zero during active transfers.
  - **Regression & Concurrency Verification**: Added tests in `ShareRoutesTest` verifying that `/cancel` aborts all concurrent uploads in a multi-file batch and prevents any file commits, and in `TransferUseCaseTest` confirming state preservation and synchronized session re-registration.
- **[fix] Desktop unique destination filename anchoring & directory relativePath preservation (`ReceiveStorage.kt`, `ReceiveStorageTest.kt`)**:
  - **Directory-Only Relative Path Filename Anchoring**: Fixed destination resolution in `ReceiveStorage.uniqueDest`. Previously, when `relativePath` specified a directory without a filename (e.g. `"documents/reports"` or `"DCIM/"`), `uniqueDest` resolved the folder itself as the file destination (`downloads/documents/reports`), completely dropping `fileName` (`"report.pdf"`). `uniqueDest` now detects directory paths and trailing slashes, appending `safeName` as the terminal file leaf (`downloads/documents/reports/report.pdf`) while preserving existing file-level relative paths (`"DCIM/photo.jpg"`).
  - **Regression Verification**: Added unit tests in `ReceiveStorageTest` confirming that directory-only relative paths and trailing-slash paths anchor `fileName` correctly without path traversal risk.
- **[fix] Cross-platform WAN relay zero-byte size integrity & unbounded stream protection (`DesktopPlatformEngine.kt`, `WanRelayClient.kt`, `WanDownloadWorker.kt`, `WanRelayClientTest.kt`)**:
  - **Zero-Byte Size Enforcement on Desktop & Android**: Corrected post-transfer size validation in `DesktopPlatformEngine.downloadWanRelay` and Android's `WanDownloadWorker.doWork`. Previously, both sides evaluated `totalBytes > 0`, which silently bypassed size verification for declared 0-byte files (`totalBytes == 0L`) and accepted non-empty payloads as valid 0-byte downloads. Both platforms now strictly enforce `totalBytes >= 0L`, asserting that 0-byte files strictly match 0 received bytes.
  - **Unbounded WAN Stream Protection**: `WanRelayClient.download` now accepts an optional `maxBytes` upper bound parameter and aborts with `IllegalStateException` immediately when incoming decrypted plaintext exceeds the threshold, eliminating runaway streaming from misconfigured or malicious peers. `WanDownloadWorker` similarly guards its frame processing loop against streaming beyond declared `totalBytes`.
  - **Zero-Byte Round-Trip & Overflow Verification**: Added tests in `WanRelayClientTest` confirming zero-byte transfers round-trip cleanly through the E2EE relay and verify that streams exceeding `maxBytes` abort early.
- **[fix] Android batch download worker size verification, stream bounds & channel position reset (`BatchDownloadWorker.kt`, `BatchDownloadWorkerTest.kt`)**:
  - **Zero-Byte File Size Integrity**: Corrected `sizeMatches` in `BatchDownloadWorker.kt`. Previously, `expected <= 0L || received == expected` evaluated to `true` when `expected == 0L`, causing the worker to accept non-empty stream data (e.g. 100 bytes received for a declared 0-byte file) as valid downloads and commit them as corrupted files. The logic now strictly requires `if (expected < 0L) true else received == expected`, ensuring 0-byte files strictly require 0 received bytes.
  - **Channel Position Reset on Transport Retry**: In `downloadOne`, explicitly reset `channel.position(0)` alongside `channel.truncate(0)` before attempting subsequent transports (QUIC to HTTP fallback), preventing non-zero file pointer offsets and offset drift across transport retries. Flush channel to disk via `channel.force(false)` before committing completed downloads.
  - **Unbounded HTTP Stream Protection & Heap Allocation**: `httpDownload` replaces off-heap `ByteBuffer.allocateDirect` with heap `ByteBuffer.allocate(65536)` to eliminate native off-heap memory fragmentation on Android, and enforces an upper bound check (`downloaded > file.size` when `file.size >= 0L`) to immediately abort runaway or misconfigured HTTP streams and protect storage from exhaustion.
- **[fix] Android file share manager non-blocking explorer, sanitized names & pull lifecycle (`FileShareManager.kt`, `FileShareManagerTest.kt`)**:
  - **Non-Blocking File Explorer & Thumbnail Decoding**: Offloaded `LIST_SHARED_FOLDERS` and `BROWSE_FOLDER` request handlers in `FileShareManager.handleRequest` to `scope.launch(Dispatchers.IO)`. Android SAF ContentResolver queries and thumbnail generation for up to 30 images (`BitmapFactory.decodeStream`, scaling, and JPEG compression) execute concurrently in coroutines instead of synchronously blocking the WebSocket client listener thread, preventing protocol stalls, ping timeouts, and UI jank.
  - **Pull Foreground Service Guaranteed Teardown & Map Eviction**: Wrapped pull transfer execution (`replyPull`) in a strict `try ... catch ... finally` lifecycle block. Unexpected exceptions or coroutine cancellations guarantee notification teardown via `PullForegroundService.stop(context)`, dispatching a terminal `PULL_REPLY` / `PULL_PROGRESS` frame to the PC and evicting tracking entries from `cancelledRequests` and `lastProgressReport` maps, eliminating foreground notification leaks in the Android status bar and memory leaks.
  - **Path Traversal & Blank Filename Sanitization**: Hardened `resolveName` and added `sanitizeName` to strip directory traversal sequences (`../`, `..\`), path separators, and illegal filesystem characters from incoming file names, falling back safely to `"unnamed_file"` if names are empty or all-whitespace.
- **[fix] Desktop upload integrity, size enforcement & pull-cancel routing (`ShareRoutes.kt`, `MessageHandler.kt`, `ShareRoutesTest.kt`, `MessageHandlerTest.kt`)**:
  - **Zero-Byte File Upload Validation**: Corrected `ShareRoutes.kt` upload validation where uploads for declared 0-byte files (`fileMeta.size == 0L`) bypassed length verification due to a faulty `fileMeta.size > 0L` condition, allowing non-empty bodies to be committed to disk and answered 200 OK. The route now strictly asserts `stagedBytes == fileMeta.size` across all file sizes.
  - **Oversized Upload Protection & Staging Eviction**: Read loop in `ShareRoutes.kt` enforces upper bound check against `fileMeta.size`, throwing `IllegalArgumentException` and responding with `400 Bad Request` if received stream bytes exceed declared size, guarding the desktop against disk exhaustion and unbounded streaming. Corrupted, oversized staging files (`stagedBytes > fileMeta.size`) are evicted and deleted via `TransferCheckpointRegistry.discardPartFile` rather than lingering permanently as unresumable disk waste.
  - **Zero-Allocation Channel Stream Reading**: Replaced per-chunk `ByteReadPacket` heap allocations (`channel.awaitContent()` + `channel.readRemaining(...)`) with direct `channel.readAvailable(buffer, 0, buffer.size)` into the reusable 256KB buffer while honoring coroutine cancellation via `currentCoroutineContext().ensureActive()`.
  - **Shared MessageHandler Pull-Cancel Routing**: Added missing `MessageTypes.PULL_CANCEL` message type to `MessageHandler.kt` in `core/network` (`when (type)` block), ensuring cancellation requests initiated from desktop file explorer (`FileExplorerService.cancelPull`) are properly forwarded to `engine.handleFileExplorerRequest(type, data)` and cancel active transfers on the peer.
...
- **[fix] WAN relay chunk stream buffering, direct channel reads & platform staging sanitization (`WanRelayClient.kt`, `DesktopPlatformEngine.kt`, `PunchSession.kt`, `WanRelayClientTest.kt`)**:
  ...
  - **Desktop WAN Staging & Path Sanitization**: `DesktopPlatformEngine.downloadWanRelay` sanitizes incoming `fileName`, `sessionId`, and `transferId` via `ReceiveStorage.sanitizeFileName`, preventing Windows reserved device collisions (`CON`, `PRN`, `AUX`, `NUL`) and path traversals, and ensures staging directory creation (`part.parentFile?.mkdirs()`) before opening output streams.
  - **Android NAT Hole-Punch MediaStore Completion**: `PunchSession.handleIncoming` calls `SafStorage.finishMediaStoreUri(context, docUri)` upon successful file reception, clearing `IS_PENDING = 1` so received downloads are immediately published to Android media scanners and external applications.
- **[fix] Desktop pull stream URL encoding, zero-allocation reads & pre-flight connect validation (`DesktopPullService.kt`, `DesktopPullServiceTest.kt`)**:
  - **URL Parameter Encoding**: `DesktopPullService.pullOne` URL-encodes both `file.fileId` and `file.token` (UTF-8) before constructing download URLs (`https://$senderIp:$httpsPort/download/$encodedFileId?token=$encodedToken`). Cryptographic tokens with Base64 characters (`+`, `/`, `=`) and file IDs with spaces or punctuation are properly received by Ktor HTTP servers rather than decoding `+` into space and failing authentication with 403/404.
  - **Pre-Connect File Staging Protection**: `DesktopPullService.streamToFile` queries HTTP status and validates `response.status.isSuccess()` before creating or truncating `.part` files on disk, preventing zero-byte staging files and disk lock contentions during 404/403 rejections or unreachable peers.
  - **Zero-Allocation Direct Channel Reads**: Stream read loop replaces packet allocations (`readRemaining(COPY_BUFFER)`) with direct `channel.readAvailable(buffer, 0, buffer.size)` on the reusable byte array, eliminating continuous `ByteReadPacket` heap churn on large multi-gigabyte transfers while honoring coroutine cancellation via `ensureActive()`.
  - **Staging File ID Sanitization**: Staging `.part` filenames sanitize `file.fileId` through `ReceiveStorage.sanitizeFileName`, preventing filesystem IO failures on Windows from illegal filename characters.
- **[fix] Download worker session lifecycle, MediaStore path specification & nested commit safety (`BatchDownloadWorker.kt`, `SafStorage.kt`, `SafStorageTest.kt`, `ReceiveStorage.kt`, `ReceiveStorageTest.kt`)**:
  - **Batch Download Worker Lifecycle & UI Cleanup**: User cancellation now explicitly sets `TcpDownloadService.updateState(isDownloading = false, error = "Transfer cancelled")` instead of leaving the download overlay stuck in an active downloading spin. An unexpected exception during a batch download now catches cleanly, purges partial documents and pending MediaStore records, updates download state, and records failed transfer history.
  - **Concurrent Task Isolation**: Isolated child file downloads inside `BatchDownloadWorker`'s coroutine scope with dedicated try/catch error reporting so that a transient failure in one file does not abort sibling concurrent downloads in the same batch.
  - **Channel & Stream Resource Scoping**: Wrapped `FileOutputStream` in a `use` block inside `BatchDownloadWorker.downloadOne`, guaranteeing proper stream flushing and file descriptor channel closure.
  - **Android MediaStore Relative Path Compliance**: `SafStorage.buildRelativePath` guarantees that `RELATIVE_PATH` strictly ends with a trailing slash (`/`) across all root and nested relative paths, fulfilling the Android MediaStore specification and eliminating `IllegalArgumentException` on Android Q+ OEM variants.
  - **Nested Storage Commit Directory Creation**: `ReceiveStorage.safeCommit` creates parent directories (`free.parentFile?.mkdirs()`) before performing file renames or cross-volume copy operations, preventing `FileNotFoundException` failures when committing nested folder transfers.
...
- **[fix] Transfer destination integrity, atomic commit & stream resumption (`ReceiveStorage.kt`, `DesktopPullService.kt`, `DesktopPlatformEngine.kt`, `ClientEngine.kt`, `DesktopFileSendService.kt`, `TransferHistory.kt`)**:
  ...
  - **Resumption Skip Integrity**: `ClientEngine.skipFully` (common core & Android client) guarantees exact stream advancement even when underlying `InputStream.skip` returns 0 (buffered streams and Android content resolvers), with EOF protection preventing silent stream truncation.
  - **Windows Path Separator Normalization**: `DesktopFileSendService` extracts clean file names via `File(path).name` rather than assuming POSIX `/` separators, ensuring Windows paths display properly in the send telemetry header.
  - **Decoupled TransferHistory Injection**: `TransferHistory` gracefully handles missing Koin application graphs in tests and offline environments, updating in-memory state and allowing test DataStore injection via `overrideDataStore` without throwing unhandled `IllegalStateException`.
- **[fix] Receiver integrity: commit, length and collision safety (`ShareRoutes.kt`, `TransferCheckpointRegistry.kt`, `ShareRoutesTest.kt`, `TransferCheckpointRegistryTest.kt`)**: An incoming upload now requires a SUCCESSFUL staging commit before any success side effect (relay registration, dedupe indexing, history, progress, HTTP 200), so a failed commit can no longer be reported as a delivered file. A body that ends cleanly but short of the declared size is refused (staging is kept for resume), an offset that cannot be honoured against the staged bytes is answered 409 instead of silently restarting from zero, staging files are unique per file id (two same-named files in one batch no longer share a `.part` and interleave), and a commit NEVER deletes an existing destination — it claims a free name or disambiguates to `name (1).ext`. Staging and its checkpoint survive every failure, so an interrupted upload stays resumable instead of becoming an untracked `.part` the sweeper cannot see.
- **[fix] Onward relay manifests keep deduplicated files (`ShareRoutes.kt`, `RelayService.kt`, `RelayServiceTest.kt`)**: Files answered `[SKIP]` at prepare time are now tracked per relay session and merged into the A → PC → B manifest. A partially deduplicated batch no longer forwards a partial file list, and an entirely deduplicated batch no longer resolves an empty manifest and fails with `relay-error` although every file was already on the PC.
- **[fix] Cancelled uploads stop writing (`ControlRoutes.kt`, `ShareRoutes.kt`, `ShareRoutesTest.kt`)**: `/cancel` now cancels the live upload handler instead of only deleting bookkeeping. The read loop and the commit path both re-check cancellation, so a cancelled transfer can neither land its bytes nor republish itself as successful, and the staging file stays resumable.
- **[fix] Pairing transitions bound to the pending peer (`PairingEngine.kt`, `WebSocketRoutes.kt`, `PairingEngineTest.kt`)**: `pair-response` and `pin-digit-entered` are applied only when they arrive from the fingerprint that owns the pending offer (`PairingEngine.isPendingPeer`). A stale rejection from a superseded pairing — or any unpaired LAN peer asserting `accepted=false` — can no longer abort the pairing in progress, and no other session can drive the PIN panel. Acceptance was already PIN-bound; rejection is now bound just as tightly.
- **[fix] Inbound `/ws` control-plane trust gate (`WebSocketRoutes.kt`)**: `set-clipboard`, `pull-progress`, `telemetry`, the `*-reply` family, `device-roster`, `peer-endpoint` and `relay-transfer` now require a live TRUSTED session (paired token, same-account proof, or PIN). Previously any socket that had merely connected — including one that never presented a fingerprint — could overwrite the desktop clipboard, forge request replies, poison punch rendezvous, or have the PC host and push staged files at a trusted target. The gate reads LIVE trust, so a session upgraded mid-flight by PIN or identity-proof is not penalised.
- **[fix] WebSocket slot cleanup is ownership-checked (`WebSocketConnectionManager.kt`, `WebSocketRoutes.kt`, `WebSocketConnectionManagerTest.kt`)**: `unregister` removes a fingerprint slot only when the calling connection still owns it, so a superseded connection's delayed cleanup can no longer evict the newer connection that took the slot (which broke its prompts, roster and connection status).
- **[fix] Android WAN downloads verify the advertised size (`WanDownloadWorker.kt`)**: A relay stream that ends cleanly BETWEEN frames is now rejected as truncated and cleaned up, instead of being recorded as a successful, silently cut file. The desktop receiver already enforced this (`DesktopPlatformEngine.downloadWanRelay`); the two platforms no longer disagree about what "delivered" means.
- **[fix] Desktop upload retries & relay fallback (`DesktopFileSendService.kt`, `DesktopFileSendRetryTest.kt`)**: Whole-batch transport failures now trigger capped exponential retries and fall back to relay transfer upon exhaustion, while HTTP auth/rejection errors fail fast immediately without retrying.
- **[fix] Desktop transfer ownership & session lifecycle (`DesktopFileSendService.kt`, `DesktopTransferOwnershipTest.kt`)**: Sending process holds session ownership until hosted delivery completes; cancellation preserves ownership during cleanup to prevent race conditions; duplicate sends cannot overwrite active transfer state.
- **[fix] Hosted download stream lifecycle & completion tracking (`RelayService.kt`, `ShareRoutes.kt`, `RelayServiceTest.kt`)**: Hosted pushes are awaited through completion or timeout; cancelling the awaiting caller revokes tokens and cancels active response streams; transfer completion is recorded only after the full payload has been flushed.
- **[fix] Durable Android transfer manifest persistence (`UploadManifestStore.kt`, `UploadWorkRequestFactory.kt`, `UploadWorker.kt`, `PunchSendWorker.kt`)**: WorkManager tasks reference app-private, work-ID-keyed URI manifests instead of unbounded JSON blobs in WorkManager Data, preventing TransactionTooLargeException; manifests survive retries and are purged upon terminal work states.
- **[fix] Off-thread Android manifest queueing (`Navigation.kt`, `ShareTargetActivity.kt`, `PendingShareForwarder.kt`)**: Picker, share-target, and pending-share operations dispatch manifest creation and WorkManager enqueueing on background dispatchers, preventing UI stalls during large multi-file shares.
- **[fix] Concurrent Android selection-size calculation (`SelectionSizeCalculator.kt`, `MainScreen.kt`, `SelectionSizeCalculatorTest.kt`)**: File size calculations snapshot selection and cache immutably, check cancellation after provider calls, and publish updates safely to the main thread for the matching selection set.
- **[fix] CI/CD validation & containerized deployment repair (`validate.yml`, `release-desktop.yml`, `server-deploy.yml`, `Dockerfile`, `deploy.sh`)**: Script execution uses explicit shells; Windows release actions target gradlew.bat; CI validation runs core modules and Android unit tests; server deployments select pre-built Docker target jars while failing fast if artifacts are missing.

### Changed
- **[minor] Screen mirroring paused (`PausedFeatures.kt`, `WebSocketRoutes.kt`, `MessageHandler.kt`, `MirrorWindow.kt`, `MainMenuColumn.kt`, `AndroidPlatformEngine.kt`, `PausedFeaturesTest.kt`)**: Screen mirroring is not an immediate feature and is now deliberately frozen rather than half-enabled. `/ws` drops `mirror-start` / `mirror-config` / `mirror-stop` and inbound binary frames at the boundary (nothing is decoded, nothing reaches the engine), the Android capture entry point is a documented no-op (no MediaProjection consent, no encoder), the desktop mirror window renders the pause notice instead of a stream, and the dock toggle no longer activates it. The new `PausedFeatures` registry records WHAT is paused, WHY, and the exact resume checklist (inbound trust gate, capture-side trust check, frame rate/size bounds, UI entry point) that must be executed before the flag is flipped back on; `PausedFeaturesTest` fails if the pause is reverted without that work.
...
- **[fix] Relocate Expanded Panel Close Button from QuickActionBar to Dock Top-Right Corner (`TopActionsPanel.kt`, `QuickActionBar.kt`)**:
  ...
  - **Top-Right Dedicated DeXCloseButton (`TopActionsPanel.kt`)**: Positioned the centralized `DeXCloseButton` (`size = DeXCloseButtonSize.Small`, `subtleColors()`) at the top-right corner of the dock card inside the `DragPillHandle` row (`Alignment.CenterEnd`). Equipped with fluid spring scale/fade entrance and exit kinematics (`DynamicMotionConfig.Default`) driven by `controller.isExpanded`, providing a natural and accessible dismiss target for expanded subpanels (Settings, File Explorer, Device Details) with zero drag handle interference.
- **[fix] Relocate Overlays Testing Lab to Settings & Centralize Dedicated Multi-Sized DeXCloseButton (`DeXCloseButton.kt`, `SettingsPanel.kt`, `TopActionsPanel.kt`, `OverlayTestingPlayground.kt`, `FluidOverlaySurface.kt`, `QuickLookModal.kt`, `PullProgressDock.kt`, `PinPairingPanel.kt`, `DeviceStatusPanel.kt`, `FileExplorerPanel.kt`, `MirrorWindow.kt`)**:
  - **Relocation of Overlays Button to Settings Developer Tools (`SettingsPanel.kt` & `TopActionsPanel.kt`)**: Removed the temporary `Overlays` button and `overlayManager` dependency from the top dock bar (`TopActionsPanel.kt`). Added an "Overlay Testing Lab" item under the "Developer Tools" section of `SettingsPanel.kt`, enabling an inline collapsible test container (`OverlayTestingPlayground`) for rapid live verification of toasts, banners, and overlays without obstructing the desktop workspace or cluttering the primary dock bar, easily removable prior to shipping.
  - **Standardized & Centralized Dedicated Close 'X' Button (`DeXCloseButton.kt`)**: Created `DeXCloseButton` in `:core:designsystem` with five distinct sizing presets (`Micro` 18dp/10dp icon, `Small` 24dp/12dp icon, `Medium` 28dp/14dp icon, `Large` 32dp/16dp icon, `ExtraLarge` 40dp/20dp icon), accessible semantics (`Role.Button`), `PointerIcon.Hand` hover cursor, tactile press kinematics via `bubbleFluidity(DeXCloseButtonDefaults.Fluidity)`, and optical specular rim lighting (`shinyGlare`). Includes color presets (`subtleColors`, `ghostColors`, `glassColors`, `dangerColors`) with zero liquid-glass or backdrop dependencies for universal safe usage.
  - **Cross-Surface Close/Dismiss Button Standardization**: Eliminated dozens of lines of ad-hoc Box/Icon/interaction boilerplate across the application by adopting `DeXCloseButton`:
    - `OverlayTestingPlayground.kt`: Replaced raw "Close" text with `DeXCloseButton(size = Small)`.
    - `FluidOverlaySurface.kt`: Upgraded the hover close pill to `DeXCloseButton(size = Small, colors = subtleColors())`.
    - `QuickLookModal.kt`: Upgraded modal header close button to `DeXCloseButton(size = Medium, colors = glassColors())`.
    - `PullProgressDock.kt`: Upgraded transfer cancellation button to `DeXCloseButton(size = Small, colors = ghostColors())`.
    - `PinPairingPanel.kt`: Upgraded header cancel button to `DeXCloseButton(size = Medium)`.
    - `DeviceStatusPanel.kt`: Upgraded panel header close button to `DeXCloseButton(size = Small)`.
    - `FileExplorerPanel.kt`: Upgraded error banner dismiss button to `DeXCloseButton(size = Micro, colors = dangerColors())`.
    - `MirrorWindow.kt`: Upgraded screen mirror disconnect button to `DeXCloseButton(size = Large, colors = glassColors())`.
- **[fix] Natural Entry Kinematics & Dynamic Island Bloom Lifecycle Across All Notification Surfaces (`FluidOverlaySurface.kt`, `NotificationBanner.kt`, `FluidNotificationStack.kt`, `OverlayPhysics.kt`, `PullProgressDock.kt`, `FileExplorerPanel.kt`, `ActiveTransferDashboard.kt`, `CornerOverlayHost.kt`)**:
  - **Dynamic Spring Entrance on Foundational Surface (`FluidOverlaySurface.kt`)**: Implemented dynamic spring entrance kinematics (`dampingRatio = 0.58f, stiffness = 320f`) into `FluidOverlaySurface`. Every notification surface (Banners, Alert Dialogs, Message Toasts, Confirmation Popups, Stacked Screens) now enters with authentic Apple Dynamic Island overshoot (`scale: 0.88f -> 1.0f -> ~1.035x overshoot`, settling elastically at `1.0f`), synchronized alpha fade (`0f -> 1f`), and physical upward glide (`16dp -> -1.6dp -> 0dp`).
  - **Natural Dynamic Island Entry Lifecycle (`NotificationBanner.kt`)**: Eliminated mouse hover requirements for notification details. Rich notifications (transfers, file shares, announcements with progress or subtitles) naturally blossom into view in `BannerMorphState.Expanded` upon arrival to display file names, speed metrics, ETA, preview thumbnails, progress bars, and action buttons immediately without requiring hover or clicks. After an organic announcement window (~3.5s), if it is an ongoing background activity, the container fluidly contracts to `BannerMorphState.Compact` (ambient pill) using `OverlayPhysics.SizeMorphDpSpring`. Tap/click toggles between expanded and compact states at any time and prevents auto-contract fighting.
  - **Refined Size Morphing Kinematics (`OverlayPhysics.kt`)**: Refined `MORPH_DAMPING` from `0.70f` to `0.58f` and stiffness to `300f` for `SizeMorphDpSpring` and `SizeMorphFloatSpring`, delivering subtle bouncy overshoot (~1.03x–1.05x) when cards expand and contract.
  - **Stack Item Composition Identity (`FluidNotificationStack.kt` & `CornerOverlayHost.kt`)**: Wrapped notification stack items in `key(item.id)` and passed stable `data.id` to `NotificationBanner`, preserving individual composition identity so existing notifications cascade smoothly across shelf peek levels when a new notification springs into view without re-triggering entrance animations or resetting morph state.
  - **Eradication of LinearSlideSpec Across In-App Transfer Entries (`FileExplorerPanel.kt` & `ActiveTransferDashboard.kt`)**: Replaced flat linear animations (`DockCardAnimations.LinearSlideSpec` and `LinearFadeSpec`) with fluid overshoot spring kinematics (`dampingRatio = 0.58f, stiffness = 320f`) for vertical slides and scale pop-in (`0.90f -> 1.0f`). Added physical drop shadow elevation (`8.dp` / `6.dp`) and `shinyGlare(RoundedCornerShape(16.dp))` optical rim lighting to `PullProgressDock.kt` and `ActiveTransferDashboard.kt`.
- **[fix] Authentic Fluent Corner Radius Morphing, Interactive Drag-to-Dismiss & Staggered Pop-In Trajectory (`DynamicPillButton.kt`, `SelectedItemsCounterPill.kt`, `IslandComponents.kt`)**:
  - **Dynamic Corner Roundness Parity (`DynamicPillButton.kt` & `SelectedItemsCounterPill.kt`)**: Eliminated forced `CircleShape` on `DynamicPillStage.FullIsland`. Added animated `currentCornerRadius` that springs from `28.dp` (exact semicircular stadium caps at 56dp height) to `48.dp` (`RoundedCornerShape(48.dp)`, matching `LiquidGlassPresets.ProfileIsland` 1:1) upon expanding to the Big Island, delivering true fluent shape morphing where bounds and corner radii interpolate simultaneously as one organic membrane.
  - **Interactive Vertical Drag-to-Dismiss Gesture (`IslandComponents.kt`)**: Equipped `ExpandedSelectionDispatchContent` with downward touch tracking featuring fluid `0.25f` rubberband resistance. Dragging past a 44dp threshold triggers haptic tick feedback (`HapticFeedbackType.TextHandleMove`) and automatically dismisses the Big Island back to the compact pill, while releasing early elastically bounces back to resting position.
  - **Apple-Grade Staggered Child Pop-In Trajectory (`IslandComponents.kt`)**: Added a 4-tier 25ms cascade with overshoot spring kinematics (`dampingRatio = 0.65f, stiffness = 380f`). The preview thumbnails, items counter telemetry, dismiss button, and target device chips pop in sequentially with coordinated scale (`0.85 -> 1.0`), opacity, and directional translations.
- **[fix] Elimination of RenderNode Hardware Recording Crash & Padding Overshoot Exception in Dynamic Island (`Navigation.kt`, `DynamicPillButton.kt`, `LiquidGlassIconButton.kt`, `IslandComponents.kt`)**:
  - **Resolution of `IllegalArgumentException: Padding must be non-negative` (`Navigation.kt`)**: Replaced `counterStartPadding` and `.padding(start = counterStartPadding)` with `counterStartOffset` and `.offset { IntOffset(x = counterStartOffset.roundToPx(), y = 0) }`. When expanding the counter pill to the Big Island, the underdamped spring (`dampingRatio = 0.50f`) overshoots past `0.dp` into negative values; using draw-layer `IntOffset` supports elastic negative overshoot without layout invalidation or Compose padding assertion failures.
  - **Resolution of Zero-Dimension Hardware Layer Recording Crash (`DynamicPillButton.kt` & `LiquidGlassIconButton.kt`)**: Replaced the permissive `if (resolvedWidth > 0.01.dp || isAnyExpanded)` condition with `if (resolvedWidth >= 1.dp && resolvedHeight >= 1.dp)` in `DynamicPillButton.kt`, and added defensive `>= 1.dp` and `.coerceAtLeast(0.dp)` guards in `LiquidGlassIconButton.kt`. This prevents `drawBackdrop` from calling `RenderNode.beginRecording(0, height)` on the initial animation frame of expansion when width interpolates up from 0.
  - **Sampling & Capture Box Decoupling (`Navigation.kt`)**: Switched `SelectedItemsCounterPill` from sampling `bottomBarBackdrop` to `sheetContentBackdrop`. Decouples the floating dock layer from `MorphingSheetNavPill`'s opacity exit spring, preventing capture texture thrashing when the navbar tabs collapse during Big Island expansion.
  - **Safe Thumbnail Overlap Layout (`IslandComponents.kt`)**: Replaced negative `Arrangement.spacedBy((-12).dp)` in `ExpandedSelectionDispatchContent` with explicit stacked start padding (`padding(start = (index * 24).dp)`) inside an explicitly sized container `Box`.
- **[fix] Tactile Press Optics Standardization Across Desktop Pairing, Device Discovery & Quick Look Surfaces (`PinPairingPanel.kt`, `InboundPairingDialog.kt`, `DeviceListPanel.kt`, `QuickLookModal.kt`, `PullProgressDock.kt`, `DragPillHandle.kt`)**:
  - **Pairing & Inbound Verification Flows (`PinPairingPanel.kt` & `InboundPairingDialog.kt`)**: Standardized `pressProgress` dynamic spring optics across the circular Close button and `PairingActionButton` in `PinPairingPanel.kt`, and the elevated modal container and Cancel button in `InboundPairingDialog.kt`. Every surface couples `bubbleFluidity(onPressedChanged = ...)` with flaring specular glare (`glareIntensity = DefaultGlareIntensity * (1f + 0.60f * pressProgress)`) and physical shadow depression (`elevation = baseElevation * (1f - 0.20f * pressProgress)`).
  - **Device Discovery & Empty-State Demonstration (`DeviceListPanel.kt`)**: Upgraded `DeviceListItemRow` and the `DeviceEmptyState` Demo Action Pill to the unified tactile press optics pipeline, adding press tracking, dynamic specular flares, physical elevation sinking, and hand hover pointers.
  - **Quick Look & Overlay Controls (`QuickLookModal.kt`, `PullProgressDock.kt`, `DragPillHandle.kt`)**: Unified the "Open App" / "Open Folder" action button, header Close button, and "Show in File Location" button in `QuickLookModal.kt`, the circular cancel button in `PullProgressDock.kt`, and the pin button in `DragPillHandle.kt` with live press springs, flaring glare, and tactile depression.
- **[fix] Tactile Press Optics Standardization Across Desktop File Explorer & Device Status Panels (`FileExplorerPanel.kt` & `DeviceStatusPanel.kt`)**:
  - **File Explorer Panel Optics (`FileExplorerPanel.kt`)**: Wired `pressProgress` dynamic spring tracking into Up-Directory circular button, SAF/History mode toggle button, and "Send Files" / "Send Folders" action pills. Each surface now executes dynamic specular glare flare (`glareIntensity = DefaultGlareIntensity * (1f + 0.60f * pressProgress)`) and shadow sinking (`elevation = baseElevation * (1f - 0.20f * pressProgress)`) with live `onPressedChanged` bubble fluidity synchronization.
  - **Device Status Panel Optics (`DeviceStatusPanel.kt`)**: Wired `pressProgress` dynamic spring tracking into the circular Close button and the bottom "Send Files" pill button, pairing `bubbleFluidity` with dynamic specular rim flares and tactile shadow depression on click.
- **[fix] Authentic 1:1 Bubble Fluidity Kinematics, Optical Press Response, Desktop Profile Restoration & QuickActionBar Parity (`BubbleFluidity.kt`, `DynamicPillButton.kt`, `BottomDockPanel.kt`, `QuickActionBar.kt`, `ModifierUtils.kt`)**:
  - **Restored Dedicated Circular Profile Button (`BottomDockPanel.kt`)**: Reverted the Desktop dock Profile button from the hover-expanding stadium pill back to a clean, non-expanding 34x34dp circular avatar button (`CircleShape`) alongside the 2-stage Exit Engine button in a single unified `Row`. Removed artificial spacer clearing hacks (`Spacer(80.dp)`), preserved instant click-to-settings (`onProfileClick`), and added support for live Google profile avatar bitmaps, initial fallback letters, and default assets.
  - **Live Optical Press Feedback Across Dock & QuickActionBar**: Plumbed `onPressedChanged = { isFluidityPressed = it }` and `interactionSource.collectIsPressedAsState()` into `BottomDockPanel.kt` (profile avatar and exit button) and `QuickActionBar.kt` (`DeXQuickActionButton`), driving animated `pressProgress` (0f -> 1f) springs with dynamic specular glare flares (`intensity = DefaultGlareIntensity * (1f + 0.60f * pressProgress)`) and tactile physical shadow sinking (`elevation = baseElevation * (1f - 0.20f * pressProgress)`).
  - **Short-Click Tactile Impulse & Velocity Preservation**: Fixed `BubbleFluidityNode` so short mouse clicks (<70ms) no longer cut off press deformation mid-flight; an internal remaining-impulse completion window guarantees the bubble reaches full squish/swell depth before releasing, and the release spring now captures and preserves `initialVelocity = baseScaleX.velocity` / `baseScaleY.velocity` across both `:core:designsystem` and `DeX/app` to deliver lively high-bouncy oscillation on every click.
  - **Public Glare Constants & Exact Token Parity**: Exposed `DefaultGlareIntensity`, `DefaultGlareWidth`, and `DefaultGlareAngle` as public constants in `:core:designsystem` `ModifierUtils.kt` for cross-module tactile parity. Aligned `LiquidGlassShadowProperties.Unexpanded` with Android's deep atmospheric 23.dp elevation (`alpha = 0.28f`, `offsetY = 18.dp`, `innerRadius = 10.dp`, `innerAlpha = 0.22f`, `innerOffsetY = 2.dp`) and `Expanded` with 28.dp floating sheet elevation.
- **[minor] Standardized DynamicPillButton Adoption for Desktop Profile Dock Button (`BottomDockPanel.kt` & `DynamicPillButton.kt`)**:
  - **Full Centralization**: Migrated the Desktop Profile button in `BottomDockPanel.kt` to the shared `DynamicPillButton` from `:core:designsystem`, eliminating ad-hoc Box/Row container logic and unifying Desktop dock kinematics with 1:1 Android parity.
  - **Authentic Anticipation Physics**: Bound `ExpansionAnchor.Start` via `rememberExpandingAnticipationPhysics`. On hover, the container executes the authentic ~70–75ms anticipation windup (elastic squish and directional nudge) before bounds morph outward to the right.
  - **Exact Android Stadium Dimensions**: Switched to `DynamicDimensions.ProfilePill` (56.dp x 56.dp collapsed circle to 210.dp x 56.dp expanded stadium pill), featuring a 34.dp avatar with a symmetrical 11.dp glass bezel when collapsed.
  - **Android Parity Slot Architecture**: Expanded slot presents a 34.dp circular avatar, 10.dp spacer, bold user display name, and standard `GOOGLE`/`DESKTOP` badge pill with native `DynamicPillDefaults.ContentTransition` and transient optical blur.
  - **Click vs. Hover Separation & Dock Baseline**: Hover drives smooth capsule expansion while clicks at any state trigger `onProfileClick` to open the Settings drawer. Unified the Exit Engine button height to 48.dp/52.dp and adjusted dock spacer clearance to 80.dp for clean horizontal baseline alignment.
- **[minor] Modular Dynamic Dismiss Button, Rolling Odometer & Apple 3-Tier Dynamic Island Spatial Regulation (`DynamicDismissButton.kt`, `RollingOdometerText.kt`, `DynamicPillButton.kt`, `SelectedItemsCounterPill.kt`)**:
  - **Modular Zero-Glass Dismiss Button (`DynamicDismissButton.kt`)**: Built a centralized, reusable dismiss/close button with tactile bubble fluidity physics (`pressScale = 0.88f`, `scalePressSpeed = 350f`, `elasticity = 0.12f`), mechanical haptic feedback (`HapticFeedbackType.TextHandleMove`), and zero liquid-glass or backdrop dependencies, allowing universal reuse across selection pills, search bars (`SheetSearchIsland.kt`), dialogs, and chips without shader overhead.
  - **Direction-Aware Rolling Odometer Physics (`RollingOdometerText.kt`)**: Implemented mechanical vertical rolling odometer animations for numeric digits using direction-aware vertical slides (`slideInVertically` / `slideOutVertically`) with overshoot spring kinematics (`stiffness = 301f`, `dampingRatio = 0.40f/0.56f`) and mechanical haptic ticks on every count increment/decrement, keeping static labels anchored.
  - **Apple-Style 3-Tier Spatial Regulation (`DynamicPillStage` in `DynamicIslandConfig.kt` & `DynamicPillButton.kt`)**: Upgraded `DynamicPillButton` to natively support 3-tier spatial states (`Collapsed` at 0.dp/56.dp, `Compact` at 56.dp capsule, `Expanded` at 180.dp stadium) with continuous stadium geometry (`CircleShape`), direction-aware dual damping, anticipation physics, and transient optical blur dissipation across tier morphs, while preserving 100% backward compatibility for binary 2-state callers.
  - **Mutual Island Regulation in Selection Counter (`SelectedItemsCounterPill.kt` & `Navigation.kt`)**: Connected media tray selection counter to the 3-tier engine: when items are selected and siblings are collapsed, it uncoils into full 180.dp stadium (`Send (N)` + `DynamicDismissButton`); when a sibling expands (Profile NamePill, History, Devices), it gracefully contracts to a 56.dp circular capsule (Send icon + mini rolling odometer badge) without disappearing; tapping the compact capsule uncoils back to full stadium; and clearing selections smoothly collapses it to 0.dp.
- **[fix] Pure Centralized Bounds-Morphing for Selected Items Counter Action Pill (`SelectedItemsCounterPill.kt` & `DynamicPillButton.kt`)**:
  - **Eradicated AnimatedVisibility & Custom Transitions**: Completely removed `AnimatedVisibility`, `expandHorizontally`, `shrinkHorizontally`, `scaleIn`, and `scaleOut` wrappers from `SelectedItemsCounterPill.kt`.
  - **Pure `DynamicPillButton` Reuse**: Connected `SelectedItemsCounterPill` directly to `DynamicPillButton` with `collapsedWidth = 0.dp` at rest and `isExpanded = (selectedCount > 0) && isExpanded && !isSiblingExpanded`.
  - **Authentic Dynamic Expansion**: When items are selected (`selectedCount > 0`), the counter pill dynamically expands from `0.dp` into view just like a button being tapped, using `DynamicMotionConfig.Default` overshoot springs (`0.50f` damping / `170f` stiffness), `ExpansionAnchor.Start` (+32dp nudge, 1.25x swell over 70ms), continuous `CircleShape` stadium capsule geometry, and real-time `LiquidGlassIconButton` backdrop refraction.
  - **Smooth Settle & Collapse**: When items are cleared or sent, it collapses smoothly back down to `0.dp` using centralized collapse springs, while preserving `lastPositiveCount` so text never flashes `Send (0)` during the exit spring.
  - **Undershoot Guard & Immediate 0dp Uncoil**: Updated `DynamicPillButton` with `shouldExpandBounds = isExpanded && (!anticipation.enabled || anticipationState.canExpandBounds || collapsedWidth == 0.dp)` and coerced `resolvedWidth = currentWidth.coerceAtLeast(0.dp)` to prevent negative Dp spring undershoots while allowing immediate uncoil when expanding from zero.
- **[minor] Centralized Dynamic Pill Physics & Optical Blur (`DynamicFluidityConfig` & `TransientContentBlur`)**:
  - **Cross-Platform Shared Design System (`:core:designsystem`)**: Centralized `DynamicIslandConfig.kt` (including `DynamicFluidityConfig`, `DynamicMotionConfig`, `DynamicContentBlurConfig`, `DynamicDimensions`, `DynamicAnticipationConfig`, `DynamicShadowVariants`, and `DynamicPillConfig`) in `commonMain`.
  - **Centralized Parallax Scaling & Jelly Physics (`BubbleFluidity.kt`)**: Upgraded `Modifier.bubbleFluidity` in `:core:designsystem` and `DeX/app` to accept `DynamicFluidityConfig`, adopting the signature tactile parameters (`pressScale = 1.08f`, `pullFactor = 0.14f`, `elasticity = 0.13f`, `scalePressSpeed = 288f`, `scalePressDamping = 0.35f`, `scaleSettleSpeed = 300f`) along with multi-axis jelly stretch kinematics (`stretchX`, `stretchY`) and release damping adjustment.
  - **Zero Desktop Screen Rewrites**: Because all desktop buttons (`QuickActionBar`, `DeviceListPanel`, `BottomDockPanel`, `PinPairingPanel`, `FileExplorerPanel`) already call `.bubbleFluidity()`, the entire Windows UI immediately inherited the dynamic pill tactile feel without screen-level code changes.
  - **Centralized Transient Optical Blur (`TransientContentBlur.kt`)**: Created `Modifier.transientContentBlur(trigger, config, motion)` for smooth 8dp optical blur dissipation during rapid structural transitions, eliminating duplicate manual `Animatable` boilerplate across `DynamicPillButton.kt` and `Navigation.kt`.
  - **Non-Expandable Button Support (`LiquidGlassIconButton.kt`)**: Upgraded `LiquidGlassIconButton` to accept `fluidity: DynamicFluidityConfig` and `contentBlur: DynamicContentBlurConfig`, wrapping child content with `transientContentBlur`.
- **[minor] Dynamic Island Lab — Companion Button Made Fully Expandable (`DynamicIslandPlayground.kt`)**: Replaced the static non-expanding companion `LiquidGlassIconButton` with a second `DynamicPillButton` sharing the exact live-tuned kinematics (`liveMotion`, `liveFluidity`, `livePillExpandedFluidity`, `liveShadows`, `liveColors`, `liveContentBlur`) so every playground slider now drives both pills. Added `isCompanionExpanded` state, mutual exclusivity (expanding one pill collapses the other), extended the screen-level outside-tap dismissal scrim to cover both expanded states, and preserved the companion's original toggle identity (tap flips the Check action state; a dedicated toggle action button lives inside the expanded "Companion Pill" stadium capsule).
- **[fix] Audit Remediation — Server Relay Quota Release, Punch Identity Hardening, Cleartext Leak & Multi-Stage Dockerfile**:
  - **Relay Drop Cleanup Guarantee (`server/RelayRoutes.kt`)**: Placed `try ... finally { RelaySessionRegistry.close(sessionId) }` inside `call.respondBytesWriter` streaming coroutine, ensuring that whether the receiver finishes draining normally or abruptly disconnects/drops mid-stream, the session is promptly closed, cancelling the underlying frame channel to immediately unblock any suspended sender and freeing the tenant's concurrent session quota slot (`MAX_CONCURRENT_SESSIONS_PER_TENANT = 2`). Added route and registry unit tests in `RelayAndPunchRoutesTest`.
  - **Mutual Identity Proof Hardening (`PunchCrypto` & `PunchCryptoChannel`)**: Upgraded punch handshake mutual identity proof from unsalted, publicly-deriveable `identityHash` (derived from email) to `identitySecret` prioritizing unadvertised `googleSub` (`deviceConfig.googleSub.ifBlank { deviceConfig.identityHash }`). Defeats on-path MITM spoofing by known contacts on hostile hotspots. Added regression test `testGoogleSubHardeningDefeatsKnownEmailIdentityHashForgery` in `:core:data` and `googleSubHardeningRejectsEmailHashAttacker` in `DeX/app`.
  - **Eliminated Cleartext Wire Leak (`PunchHelloDto`)**: Removed cleartext `sessionId` from `PunchHelloDto` transmissions over unencrypted initial TCP packets. Direct NAT punch handshakes reveal zero metadata to passive network observers; session IDs and file manifests ride exclusively inside authenticated, sequence-bound AES-256-GCM frames.
  - **Multi-Stage Server Dockerfile & `.dockerignore`**: Upgraded `server/Dockerfile` to a self-contained multi-stage build using `eclipse-temurin:21-jdk-alpine` builder and `eclipse-temurin:21-jre-alpine` non-root runtime (`dex` user), enabling zero-dependency VPS builds directly from `git clone`. Added root `.dockerignore` excluding build artifacts, legacy WPF assets, and Android modules from build context.
- **[fix] Wire protocol compliance & envelope parsing hygiene (Audit Remediation)**: Added `STREAM_TOKEN`, `RELAY_URL`, `FILE_NAME`, `SIZE`, and `PEER_FINGERPRINT` to `FieldNames` in `:core:protocol` and forwarded them in `DeX/app`'s `ProtocolKeys` shim; added golden fixture test `relay_offer_envelope_is_canonical`; eliminated hand-rolled envelope decoding in `MessageHandler.kt` and `WebSocketRoutes.kt` via `ProtocolEnvelope.decodeType`/`decodeData`; replaced raw string literals (`"sessionId"`, `"streamToken"`, `"relayUrl"`, `"fileName"`, `"size"`, `"fingerprint"`, `"targetFingerprint"`, `"peerFingerprint"`) across `WebSocketRoutes.kt`, `DesktopFileSendService`, `WanRelayClient`, `DiscoveryEngine`, `DesktopUdpService`, and `:server`'s `RelayRoutes` with `FieldNames.*`; and updated `docs/ARCHITECTURE.md` to document the KMP `androidMain` targets.
- **[fix] Android punch resume-state memory leak (ROBUST-PUNCH-02)**: Declined or prompt-timed-out punch transfers leaked their per-session resume map forever — `PunchResumeState.mapFor(sessionId)` runs before the acceptance check in `PunchSession.handleIncoming`, but `prune()` only sweeps sessions that were actually accepted. Added `PunchResumeState.discard(sessionId)` and wired it into the declined/timeout reject path; regression test added to `PunchResumeStateTest`; `:app:testDebugUnitTest` green.
- **[minor] Deferred-finding closure: println to structured logging (DX-02)**: Migrated the only production-code `println` calls (`core/designsystem/icons/DeXAnimatedIcons.kt` Lottie asset-failure catch blocks) to Kermit `Logger.e` with a `DeXAnimatedIcons` tag, matching the established logging convention. Remaining `println` uses documented as deliberate in the findings ledger: the server's pre-exit `System.err` FATAL line (emergency channel before any logging exists), manually-run `scripts/*.kts` dev tools, and one Android unit test. `:core:designsystem` compile verified green.
...
### Added
- **[minor] Direct Big Island Expansion on Selection Counter Pill & Target Device Chooser (`SelectedItemsCounterPill.kt`, `DynamicPillButton.kt`, `DynamicIslandConfig.kt`, `Navigation.kt`)**:
  - **Direct Pill Morphing & Roller Isolation**: Directed Big Island expansion to occur directly on `SelectedItemsCounterPill` itself rather than commandeering the bottom-right profile roller button; the profile roller remains untouched and strictly dedicated to profile/transfer modes.
  - **4-Tier Dynamic Island Spatial Regulation (`DynamicPillStage.FullIsland`)**: Upgraded `DynamicPillStage` with `FullIsland` and equipped `DynamicPillButton` with `fullIslandWidth`, `fullIslandHeight`, `fullIslandGlassConfig`, and `fullIslandContent` slots, enabling any dynamic pill to morph seamlessly from 0.dp collapsed or 56.dp compact into an expansive `(screenWidth - 32.dp) × 140.dp` dynamic island container with Apple-style overshoot springs.
  - **Dynamic Start-Padding & Z-Index Elevation (`Navigation.kt`)**: Animated `counterStartPadding` from `66.dp` down to `0.dp` via spring physics during Big Island expansion, elevating container z-index to `35f` and fading `navPillAlpha` to `0f` so the counter pill gracefully blooms across the full dock width without off-axis drift or overflowing the screen bounds.
  - **Tactile Tap-and-Hold Trigger**: Integrated `combinedClickable(onClick, onLongClick)` across both compact and expanded states of the counter pill; tap triggers immediate default send while long-press triggers mechanical `HapticFeedbackType.LongPress` and blooms into the Big Island.
  - **Self-Contained Dispatch & Dismissal**: `fullIslandContent` embeds `ExpandedSelectionDispatchContent` directly inside the expanded counter pill, displaying thumbnail stack, item count, formatted byte telemetry, and destination device chips for instant dispatch. Dismiss button ('✕'), outside-tap scrim, and Android predictive back gesture seamlessly collapse the Big Island back to the 180.dp stadium pill while preserving selections intact.
- **[minor] Total Payload Size Telemetry for Dynamic Selection Counter Pill (`SelectedItemsCounterPill.kt`, `Formatters.kt`, `Navigation.kt`)**:
  - **Asynchronous Background Size Calculation (`Navigation.kt`)**: Wired a coroutine-backed URI size resolution system using `Dispatchers.IO` and an in-memory session cache (`uriSizeCache`), asynchronously summing total selected bytes without dropping frames on the Compose UI thread even during multi-selection bursts.
  - **Centralized Robust File Size Resolver (`Formatters.kt`)**: Added `Formatters.resolveFileSize(context, uri)` supporting both `content://` (via `OpenableColumns.SIZE` query and `openFileDescriptor` stat fallback) and `file://` (via direct `File.length()`), eliminating duplication across `ShareSheetContent.kt` and UI components.
  - **Apple-Style Two-Line Telemetry Layout (`SelectedItemsCounterPill.kt`)**: Upgraded Tier 2 (Expanded Stadium Pill) to present a clean two-line hierarchical layout: primary action row (`"Send ("` + `RollingOdometerText` + `")"`) on top with 14.sp SemiBold, and secondary payload telemetry (`formattedSize`, e.g. `42.5 MB`) below with 11.sp 75% opacity and graceful ellipsis truncation.
  - **Seamless Collapse Preservation**: Cached `lastPositiveSize` alongside `lastPositiveCount` so telemetry text never flickers to `"0 B"` or pops abruptly during spring collapse transitions.
  - **Compact Tier 1 Cleanliness**: Preserved clean badge aesthetics in Tier 1 (Compact 56.dp Capsule), displaying icon and badge while cleanly hiding payload bytes until uncoiled.
- **[minor] Multiplatform Dynamic Pill Stadium Container (`DynamicPillButton.kt` & `ExpandingAnticipationPhysics.kt`)**:
  - **Multiplatform Stadium Primitive (`:core:designsystem`)**: Created pure-Compose `DynamicPillButton` in `commonMain`, bringing Apple-style fluid overshoot spring kinematics, tactile squish physics, dynamic multi-tier shadow morphing, native frosted surface styling, and directional specular glare (`shinyGlare`) to Compose Desktop and future KMP targets without external backdrop library dependencies.
  - **Strict Stadium Capsule Geometry**: Enforces continuous semicircular caps (`CircleShape`) at all frames, preventing deformation into cards or rounded rectangles during bounds morphing.
  - **RenderThread Anticipation Physics (`ExpandingAnticipationPhysics.kt`)**: Ported hardware-accelerated anticipation squish (1.25x), directional nudge (+32dp / -32dp based on `ExpansionAnchor`), and uncoil rebound kinematics directly into `commonMain`.
  - **Integrated Tactile Fluidity & Optical Blur**: Direct integration with `Modifier.bubbleFluidity` (1.08x swell, 0.14 drag pull, 0.13 jelly elasticity) and `Modifier.transientContentBlur` (8dp peak blur dissipation during structural state changes).
  - **Decoupled Slot API**: Pure generic Compose slots (`collapsedContent: @Composable () -> Unit`, `expandedContent: @Composable (collapse: () -> Unit) -> Unit`) with both granular inline parameter overrides and unified `config: DynamicPillConfig` composite parameter support.
- **[minor] Selected Items Counter Pill (`SelectedItemsCounterPill.kt`)**:
...
- **[minor] Desktop Action Bar, Explorer & Pairing Stadium Modernization (`QuickActionBar.kt`, `FileExplorerPanel.kt`, `PinPairingPanel.kt`)**:
  - **Dynamic Danger Close Pill Kinematics (`QuickActionBar.kt`)**: Upgraded collapsible Danger Close pill expand and shrink transitions from static springs to `DynamicMotionConfig.Default` direction-aware overshoot springs (`expandDampingRatio = 0.50f`, `collapseDampingRatio = 0.56f`, `stiffness = 170f`).
  - **Quick Action Stadium Capsule & Optical Blur (`QuickActionBar.kt`)**: Enforced strict `CircleShape` geometry on all 4 action pills (DND, Mirror, History, Clipboard) and Danger Close. Standardized non-expanding tactile behavior on `DynamicFluidityConfig.Default` (`pressScale = 1.08f`, `pullFactor = 0.14f`, `elasticity = 0.13f`), added hand cursor affordance (`PointerIcon.Hand`), and bound `Modifier.transientContentBlur(trigger = isChecked)` for smooth optical dissipation during toggle transitions.
  - **File Explorer Stadium & Non-Expanding Tactile Feel (`FileExplorerPanel.kt`)**: Upgraded 40dp Up Directory button, 40dp Search / Path indicator pill, and 40dp Mode Toggle button from legacy rounded corners to strict `CircleShape` stadium capsules. Standardized on `DynamicFluidityConfig.Default`, applied `transientContentBlur(trigger = mode)` across Phone SAF and PC History switches, added `PointerIcon.Hand`, and transformed bottom "Send Files" and "Send Folders" actions to stadium geometry.
  - **Pin Pairing Panel Stadium Standardization (`PinPairingPanel.kt`)**: Modernized 28dp header Close button and `PairingActionButton` (Cancel / Use PIN Code) to strict `CircleShape` stadium capsules, integrated `DynamicFluidityConfig.Default` tactile physics, `PointerIcon.Hand`, and `transientContentBlur(trigger = label)` during mode switches.


- **[minor] Liquid Glass History Button Kinematics, Multi-Tier Elevation & Roller Dual-Damping**:
  - **Dedicated Preset (`LiquidGlassConfig.kt`)**: Added `LiquidGlassPresets.HistoryIconButton` with `CircleShape` and `LiquidGlassShadowProperties.Unexpanded` (23dp/18dp outer, 10dp/2dp inner); active view elevates to `LiquidGlassShadowProperties.Expanded` (28dp/24dp outer, 12dp/7dp inner) with theme-aware `primary` tint.
  - **Tactile Bubble Fluidity**: Bound CodeDeX's tuned pointer physics (`pressScale = 1.08f`, `pullFactor = 0.14f`, `elasticity = 0.13f`, `scalePressSpeed = 288f`, `scalePressDamping = 0.35f`, `scaleSettleSpeed = 300f`) to `RollerButtonItem`.
  - **Icon Bounce Kinematics**: Upgraded `iconSpringScale` in `RollerButtonItem` from rigid 500f spring to CodeDeX's tuned overshoot spring (`dampingRatio = 0.40f, stiffness = 301f`).
  - **Direction-Aware Magnetic Roller Springs**: Upgraded `rollerOffset.animateTo` transitions (automatic cycle, gesture release, drag snap) to direction-aware dual damping (`0.40f` moving / `0.56f` settling, `301f` stiffness).
  - **Interactive High/Half Tier Toggle**: Tapping the History button when History screen is open toggles between `SheetTier.High` and `SheetTier.Half`.
- **[minor] Liquid Glass Segmented Control Highlighter Lens Kinematics & Multi-Tier Elevation**:
  - **Direction-Aware Dual Damping Kinematics**: Upgraded highlighter slide spring (`dampingRatio = 0.40f, stiffness = 301f`) and bulge springs (`0.40f` active / `0.56f` settling, `301f` stiffness) in `LiquidGlassSegmentedControl.kt`.
  - **Dynamic Multi-Tier Shadows**: Bound `LiquidGlassShadowProperties.Unexpanded` (23dp/18dp outer, 10dp/2dp inner) at rest and `LiquidGlassShadowProperties.Expanded` (28dp/24dp outer, 12dp/7dp inner) during touch/slide interaction, eliminating resting visual flatness and achieving authentic volumetric glass elevation.
  - **Transient Lens Blur & Optics**: Integrated dynamic 1.5dp optical blur through the highlighter during motion settling to 0dp at rest, with specular glare boost to 0.85f during drag.
  - **Tab Icon Spring Alignment**: Upgraded tab icon selection bounce to CodeDeX's tuned overshoot spring (`dampingRatio = 0.40f, stiffness = 301f`).
- **[minor] Sheet Search Island Kinematics, Multi-Tier Shadows & Transient Blur Integration**:
  - **Dynamic Pill Reusability (`SheetSearchIsland.kt`)**: Refactored the floating search island to delegate directly to `DynamicPillButton`, standardizing the stadium pill morphology and eliminating duplicated spring and shadow logic.
  - **Tuned Overshoot Kinematics**: Bound CodeDeX's direction-aware dual damping (`expandDampingRatio = 0.40f`, `collapseDampingRatio = 0.56f`, `stiffness = 301f`) to horizontal and vertical bounds expansion.
  - **Tactile Bubble Fluidity**: Bound tuned touch physics (`pressScale = 1.08f`, `pullFactor = 0.14f`, `elasticity = 0.13f`, `scalePressSpeed = 288f`, `scalePressDamping = 0.35f`, `scaleSettleSpeed = 300f`) across collapsed and expanded states.
  - **Multi-Layer Liquid Glass Shadows (`LiquidGlassConfig.kt`)**: Bound `LiquidGlassShadowProperties.Unexpanded` (23dp/18dp outer, 10dp/2dp inner) to `LiquidGlassPresets.SearchIconButton` and `LiquidGlassShadowProperties.Expanded` (28dp/24dp outer, 12dp/7dp inner) to `LiquidGlassPresets.SearchIsland` with strict `CircleShape`.
  - **Transient Optical Content Blur**: Applied 75ms rapid rise to 8dp peak blur with elastic spring dissipation during expand and collapse, rendering at zero GPU overhead when settled at rest.
  - **Interactive Search Collapse**: Tapping the search icon on the left inside the expanded pill calls `collapse()` with haptic feedback; text styling bound to `MaterialTheme.colorScheme.onSurface` for automatic light/dark theme adaptation.
- **[minor] Dynamic Island & Profile Button Kinematics, Multi-Tier Shadows, and Squircle Deletion**:
  - **Profile Island Refinement (`Navigation.kt`)**: Upgraded Profile button and expanded island morphing with CodeDeX's tuned kinematics (`expandDampingRatio = 0.40f`, `collapseDampingRatio = 0.56f`, `stiffness = 301f`).
  - **Tuned Bubble Fluidity**: Bound tactile pointer physics (`pressScale = 1.08f`, `pullFactor = 0.14f`, `elasticity = 0.13f`, `scalePressSpeed = 288f`, `scalePressDamping = 0.35f`, `scaleSettleSpeed = 300f`) to the Profile button, active across both collapsed and expanded states.
  - **Transient Optical Content Blur**: Added a 75ms transient optical blur (peak 8dp) with direction-aware spring dissipation during Profile expansion and collapse, rendering at zero GPU cost in resting states.
  - **Multi-Tier Liquid Glass Shadows (`LiquidGlassConfig.kt`)**: Bound CodeDeX's tuned multi-layer outer and inner shadows directly to `LiquidGlassPresets.ProfileIconButton` (unexpanded: 23dp/18dp outer, 10dp/2dp inner) and `LiquidGlassPresets.ProfileIsland` (expanded: 28dp/24dp outer, 12dp/7dp inner).
  - **Squircle Deletion & Playground Consolidation**: Deleted `DynamicIslandButton.kt` completely from the codebase; simplified `DynamicIslandPlayground.kt` to focus exclusively on `DynamicPillButton` stadium morphs.
  - **Zero Regression Guarantee**: Preserved `ExpandedProfileContent` layout and logic, downloading/transfer island, and roller drag gestures verbatim. All 67 Android unit tests and Desktop modules green.
- **[major] Plan 046 executed — Punch Data-Plane End-to-End Encryption (E2EE)**:
  - **Shared Cryptographic Engine (`:core:data`)**: Created `PunchCrypto` (`commonMain`, `androidMain`, `jvmMain`) providing ephemeral NIST P-256 (`secp256r1`) ECDH key agreement, HKDF-SHA256 key schedule (`"dex-punch-session-v1"`, `"dex-punch-auth-v1"`), mutual HMAC-SHA256 proof-of-identity bound to `identityHash` and ephemeral public keys (constant-time verification), and frame sealing/opening with AES-256-GCM sequence AAD binding.
  - **Android Forwarding Shim**: Added `PunchCrypto.kt` in `DeX/app/.../crypto` forwarding directly to `:core:data`.
  - **Encrypted Framed Streaming (`PunchCryptoChannel.kt`)**: Implemented length-prefixed binary frame reader and writer (`4-byte length + [12-byte nonce][ciphertext + 16-byte tag]`) with monotonic sequence numbers and bounded frame sizes (`MAX_FRAME_BYTES`).
  - **Direct Punch Upgrades (`PunchTransferChannel.kt` & `PunchSession.kt`)**: Upgraded phone-to-phone NAT punch data plane from plaintext TCP to fully authenticated and encrypted streams. Ephemeral handshake executes prior to manifest; manifests, resume offsets, file headers, 64 KiB chunks, and completion markers ride inside authenticated frames. Legacy v1 peers cleanly fail with `{"type":"reject","reason":"upgrade_required"}` without hanging or crashing.
  - **Comprehensive Test Coverage**: Added desktop test suite `PunchCryptoTest.kt` in `:core:data` (5 tests) and Android test suite in `DeX/app` (8 tests) verifying key agreement, mutual identity proof validation, rogue identity rejection, replay/reordering defenses, and framed streaming. `:core:data:desktopTest`, `:app:testDebugUnitTest`, `:core:network:desktopTest`, and `:app:assembleDebug` all passing; `spotlessCheck` passing across all 36 tasks.
- **[minor] Plan 044 executed — UI state-holder extraction, Sheet Search Island repositioning & obsolete FloatingPillNavBar cleanup (Android plan-024 Phase 4)**: Per /grill-me alignment, removed obsolete prototype `FloatingPillNavBar.kt` (511 lines) and cleaned up KDocs in `LiquidGlassSegmentedControl.kt`. Repositioned the liquid-glass bouncy expanding Search Island (`SheetSearchIsland.kt`) from the root top app bar to the top-right of the sheet card, visible exclusively when History mode is selected; retired `FloatingTopAppBar.kt` leaving the top screen clean with only the central Dynamic Island. Updated placeholder to "Search history..." and removed device search filtering on `MainScreen.kt`. Decoupled state architecture into `HistoryUiState.kt` (`HistoryState`), `TopIslandState` (profile/island expansion), and a backward-compatibility facade in `TopAppBarState`. Added `HistoryUiStateTest.kt` (7 tests); `:app:assembleDebug`, `:app:testDebugUnitTest`, and `:composeApp:desktopTest` green. Manual soak pending user.
- **[minor] Plan 043 executed — NavBottomSheet drag-state extraction (Android plan-024 Phase 4)**: Extracted pure tier geometry, dynamic gap/radii calculations, golden scrim formula (`baseScrimAlphaFor`), and drag-settle decision engine (`NavSheetDecider`) into `NavSheetModel.kt`. `NavBottomSheet.kt` simplified to delegate geometry, scrim calculation, and gesture settling directly to the extracted models with zero visual/kinematic change (verbatim thresholds: dismiss 50dp, fling velocity 350dp/s, drag commit 36dp, fling dismiss bound 30dp). Added unit test suite `NavSheetModelTest.kt` (13 tests) verifying golden values and all gesture settle actions; `:app:assembleDebug`, `:app:testDebugUnitTest`, and `:composeApp:desktopTest` green. Manual soak pending user.
- **[minor] Plan 042 executed — PunchSession split (Android plan-024 Phase 4)**: Split the 511-line `PunchSession` into verbatim-moved seams: `PunchLineProtocol` (UTF-8 newline framing with 64 KiB hostile-line drop), `PunchSocketConnector` (endpoint registration with TLS cert pinning + simultaneous-open NAT punch, socket lifecycle observed via provider so cancellation semantics are unchanged), and `PunchTransferChannel` (manifest/header framing, resume-offset streaming, completion marker). `PunchSession` retains lifecycle, receiver prompting, and send orchestration; public surface (`start`/`stop`/`sendTo`) unchanged for `DexService`, `PunchSendWorker`, and Koin wiring. All punch wire values and timers preserved byte-identically; `:app:assembleDebug` and `:app:testDebugUnitTest` green. Manual soak pending user.
- **[minor] Plans 042-045 — Android plan-024 Phase-4 entries created**: Grounded plan entries for the deferred high-risk Android refactors: 042 PunchSession split (rendezvous / transfer / line-protocol seams, wire-verbatim), 043 NavBottomSheet drag-state extraction (kinematics byte-identical, backdrop regression guard), 044 UI state-holder extraction for FloatingPillNavBar + TopAppBarState (includes a user decision gate on state-retention semantics), 045 OkHttp/Cronet client consolidation (WP0 audit of all four builder profiles incl. cert-pinning posture). All four require manual soak testing before DONE; ledger rows registered and plan 024 row updated to reference them.
- **[minor] Plan 032 load-test harness** (`server/scripts/loadtest.sh`): production load test for the deployed relay host — `/healthz` liveness, all four unauthenticated 401 gates (relay/punch/sync), the 429 quota gate enforced before first byte, concurrent relay streaming with byte-integrity verification and throughput measurement, punch register/resolve round-trip, sync exchange smoke, and an opt-in 2 GiB session-cap breach probe (`DEX_MB=2050`). Validated against a live fixture-auth server instance: liveness and auth gates pass, authenticated phases fail closed on invalid tokens.
...
- **[minor] Plan 032 CI deploy path resolved (Option A, user-approved)**: `.github/workflows/server-deploy.yml` reworked to the single canonical deploy path — CI builds and tests `dex-server-all.jar`, ships the JAR + `server/` source bundle over SSH, and the VPS executes `server/scripts/deploy.sh` (compose + Caddy auto-HTTPS), so TLS, health probing and rollback semantics can never diverge between CI and manual deploys. Server env (`DEX_GOOGLE_CLIENT_ID`, `DEX_DOMAIN`) now lives only in `/opt/dex/server/.env` on the VPS (excluded from the CI bundle, never overwritten by CI); SSH credentials remain the only GitHub secrets. Runbook updated with one-time VPS provisioning steps.
...
- **[minor] Round-3 hardening — client law mirror, streaming timeout, botnet caps, shutdown hygiene**:
  - **Client-side legal-collections law (`SyncEngine`)**: `mutate` rejects illegal collections with a contract error and `mergeIncoming` IGNORES them — the client mirror of the server's door law. A hostile/buggy host answering with unknown collections (or crafted prefix variants like `devices-x`) can no longer plant garbage rows in local storage. `DataStoreSyncStorage` adds defense-in-depth: dots are rejected in collection/key segments (namespace-collision guard for the prefix scan).
  - **WAN streaming timeout fix (transfer-killing bug I introduced)**: the download's whole-request 120s timeout aborted any transfer slower than 2 minutes total (multi-GB files over slow uplinks die mid-stream). Now connect (10s) + inactivity (60s) guards only — the pattern the LAN pull service already uses — while control-plane calls (open/complete/close/push) keep the 120s cap.
  - **Punch table GLOBAL cap (10,000)**: the per-tenant cap alone never bounded total memory — a botnet of throwaway accounts could each register 64 entries and OOM the host. New tenants are rejected first; established tenants keep refreshing (never wedged by junk).
  - **`SyncBridge.stop()`**: the bridge's supervisor scope was never cancelled — a straggler `engine.mutate` after DataStore teardown crashed as an uncaught exception in whatever ran next (the same pollution class the MessageHandler fix addressed). Now cancelled by the shutdown coordinator; late enqueues are dropped (offline-first: re-queued next session).
- **[minor] Clock-persistence write amplification fixed**: `storeClock` fired on EVERY merged record (a 5k-record bootstrap = 5k DataStore writes for one logical state change). Now persisted only when the clock actually advanced — pinned by the existing restart-causality test.

## [10.1.37.0] - 2026-09-02
### Added
- **[major] Delta Sync (cursor-based) — protocol + host + client (round-2 enhancements)**:
  - **Wire law (core/sync)**: `SyncExchangeRequest.sinceHostSeq` (absent = full snapshot — backward compatible with every deployed client) + `SyncExchangeResponse.hostSeq/hasMore` + shared `SyncLimits.MAX_RECORDS_PER_RESPONSE` (5k). One definition; host and every peer consume it.
  - **Host**: `SyncHostStore` stamps each accepted record with a per-tenant monotonic sequence; `snapshotSince(tenant, since)` returns only newer records, seq-ordered, batched with hasMore. An up-to-date client's 5-minute exchange is now an empty no-op pull instead of a full-subtree download. Route honors the cursor, bounds `deviceId` (256 chars).
  - **Client**: `SyncEngine.flush` sends the persisted cursor, drains hasMore pages immediately (bounded at 8 pages — a hostile host cannot spin the loop), and advances the persisted cursor only after a fully-merged response. `DataStoreSyncStorage` persists the cursor alongside the HLC state. Test-pinned: cursor persisted after clean flush, delta windows return only newer records, idle exchange = 0 records.

### Fixed
- **[major] HLC clock persistence (causality bug)**: the engine never persisted its advanced HLC state — after a restart, a device could issue timestamps that LOSE HLC-LWW conflicts against its OWN pre-restart records (silent data regression: your newer write reverts to your older one). Clock state is now persisted with every mutation and every merged remote record; the restart contract is pinned by a test where session 2 (restored clock) must supersede session 1 and reject session-1 replays.
- **[major] Relay frame sequencing (cryptographic replay/reorder/gap defense)**: sealed relay frames now bind their sequence number via AES-GCM AAD (`sealFrame`/`openFrame`). A duplicated frame (HTTP retry), a reordered frame, or a gap fails authentication on the receiver — previously any of these silently corrupted the file. Adversarial tests: replay-at-wrong-seq, reorder, gap, and seq-uniqueness all fail closed; the WAN client uses sequenced frames end-to-end.
- **[minor] Cancellation hygiene**: a cancelled `flush` (shutdown/scope death) no longer swallows `CancellationException` — deltas re-queue and the cancellation propagates (no zombie coroutine states).
- **[minor] Client-side response bound**: the transport refuses sync responses above 64 MiB before parsing — a hostile/buggy host cannot OOM the client.
- **[minor] Flush jitter**: the scheduler sleeps interval ± 30s randomized — a fleet of devices no longer flushes on the same wall-clock grid (thundering-herd de-sync). Test seam pins jitter to 0 for deterministic assertions.
- **[minor] Synced-history GC**: `compact()` now trims local synced HISTORY to the freshest 500 records (matching the desktop UI window) — unbounded DataStore growth otherwise. Test: 602 records → 500 freshest survive, oldest trimmed first.

## [10.1.36.0] - 2026-09-02
### Fixed
- **[major] Backend Edge-Case Hardening Pass (security, memory, concurrency, DoS)**:
  - **SECURITY — punch rendezvous auth (plan-032 law violation)**: `/punch/register` and `/punch/resolve` previously accepted ANY caller; now both require the verified Google ID token, entries are tenant-scoped (cross-account resolve returns indistinguishable 404 — no account-membership oracle), input lengths are bounded, and a per-tenant 64-entry cap with sweep-on-every-request prevents table poisoning/DoS. New tests: unauthenticated/forged rejection, cross-account isolation.
  - **Memory discipline — `WanRelayClient.download` streamed**: previously buffered the ENTIRE file in memory (a 10 GiB WAN transfer would OOM the desktop); now decrypts frame-by-frame straight to `OutputStream` via the response channel (KiB-scale memory regardless of file size). Frame parser hardened against hostile lengths (negative/oversized → fail closed mid-stream).
  - **WAN failure semantics**: `upload` now signals `/complete` ONLY on a fully-pushed stream and hard-closes the session on ANY failure (previously a failed upload completed the channel, letting the receiver accept a truncated file as complete); blank paired-token and blank target guards added; 120s request timeouts on every relay call (a wedged host fails the request instead of stalling the transfer forever).
  - **Sync exchange timeouts**: `HttpSyncTransport` gets a 90s request timeout — a hung sync host now fails the flush (engine re-queues + backs off) instead of blocking the flush loop indefinitely.
  - **Concurrency — domain state machines serialized**: `DeviceRegistry` (beacon threads + telemetry + roster + sweep could lose updates or double-evict), `TransferUseCase` (progress/complete/remove could interleave, flipping complete→in-flight or losing a removal), and `RelaySessionRegistry.openSession` (parallel opens could both pass the quota count-then-act) now guard read-modify-write sequences.
  - **SyncEngine re-queue regression fix**: a failed exchange re-queued its drained deltas unconditionally, which could OVERWRITE a newer delta queued mid-exchange; re-queue now only restores a record when no newer HLC for the same key arrived (regression test: newest write wins).
  - **Server DoS bounds**: sync exchange rejects oversized bodies (Content-Length pre-check + body-char bound) and delta batches >1000 BEFORE parsing; `SyncHostStore` caps records per tenant at 20,000 (hostile accounts can never OOM the host; existing keys always remain updatable — the cap never wedges an account).
  - **Ops — `/healthz` + Dockerfile HEALTHCHECK fix**: the old healthcheck probed an auth-gated route (always 401 → container restart-looped on a perfectly healthy server). New unauthenticated `/healthz` liveness endpoint; Dockerfile updated to probe it.
- **[minor] Concurrency load test**: four parallel relay sessions (two tenants × the per-tenant cap) pump + drain concurrently through the bounded channels — all frames arrive intact, all sessions release quota (registry-level).

## [10.1.35.0] - 2026-09-02
### Added
- **[major] E2EE Relay Crypto + WAN Orchestration (plan 032 WP2)**:
  - **`RelayCrypto` (core/data, KMP expect/actual)**: the end-to-end encryption key schedule for WAN relay transfers — HKDF-SHA256 (RFC 5869 extract-then-expand) over the paired token (IKM) + relay sessionId (salt) with "dex-relay-v1" domain separation, yielding AES-256-GCM per-chunk AEAD (random 96-bit nonces, 128-bit tags). Both peers derive the identical key; no key material ever crosses the wire — the relay stays an unknowing postman by construction. JVM actuals use javax.crypto exclusively (zero hand-rolled primitives). 10-test suite: deterministic peer derivation, per-session key uniqueness, nonce randomness, 0B–1MiB+ round-trips, wrong-key/tamper/truncation fail-closed, independent multi-chunk streams.
  - **`WanRelayClient` (core/network)**: desktop WAN transfer orchestration — authenticated relay session open, sender streams 256 KiB chunks sealed as `[4-byte length][sealed]` frames (framing rides INSIDE the opaque stream; the relay learns only chunk sizes it already quota-accounts), explicit `/complete` termination, receiver drains and opens every frame under the same derived key with zero plaintext leakage on any AEAD failure. 5 orchestration tests run against the REAL server module's relay routes (embedded), including 37-chunk framing-integrity and per-test quota hygiene.
- **[major] Server relay data-plane defect fixed (found by the orchestration tests)**: the original SharedFlow pass-through had no completion semantics — the receiver's HTTP response never terminated (infinite hang) and subscriber-less emissions were silently dropped. Rewritten as a capacity-bounded Channel (64 frames ≈ 16 MiB): an outrunning sender suspends at capacity (bounded-memory law now mechanically enforced), `/complete` closes the channel so the receiver's drain always terminates, and an idempotent `/close` releases quota immediately.

## [10.1.34.0] - 2026-09-02
### Added
- **[major] core/domain Clipboard Sync Use Case (plan 029 DONE)**:
  - **`ClipboardSyncUseCase` (core/domain/clipboard)**: the platform-neutral clipboard sync brain — the content-hash **echo guard** (received content is written locally AND hash-marked so the platform's own change event never re-broadcasts it, killing the classic two-device infinite copy loop), the enable policy (injected value source — the UI toggle drives it live), Text-XOR-Image payload lanes, and send-failure propagation so callers own fallback channels. `ClipboardAccess` + `ClipboardSender` ports; zero sync dependency by design (clipboard content is real-time P2P only — the privacy law restated in code). 8-test contract suite pins every semantic.
  - **Desktop wiring**: ONE process-wide instance (NetworkModule.jvm: AWT access + WS-broadcast sender, eagerly resolved at server start so the guard exists before any frame arrives). The server receive path (`WebSocketRoutes` `set-clipboard`) routes through the same shared instance via `ClipboardSyncState.applyRemoteText` — receive and change-listener consult the SAME echo guard. `ClipboardSyncService` is reduced to AWT flavor-listener plumbing; the Windows ADB fallback (bundled platform-tools, bounded process) lives in `DesktopClipboardSender` (WS-first, ADB only when no live session).
- **[minor] Governance**: `AuthState` disposition decided and recorded in plan 029 — it stays in `core/network` (it is the DeviceManager persistence mirror, not domain logic); plan 030's Android integration is the decision point where a shared mirror is re-evaluated with full information.

## [10.1.33.0] - 2026-09-01
### Added
- **[major] core/domain Discovery & Roster Registry (plan 028 DONE)**:
  - **`DeviceRegistry` (core/domain/discovery)**: the observed-device state machine extracted to the domain layer — fingerprint-keyed replace-on-change detection (ip/info/viaWan/viaRoster), 100-entry DoS cap with oldest-lastSeen eviction, 20s-freshness/10s-sweep TTLs, telemetry merge (battery/isCharging/wifiSsid + lastSeen refresh), roster upsert lane (`viaRoster=true`, no TTL — roster entries are not beacon-driven), and blank-fingerprint rejection. Every number and rule migrated EXACTLY from the live `DiscoveryEngine`, pinned by a 12-test contract suite.
  - **Ports**: `KnownPeerPersistence` (desktop's PcMemory already implements the shape; formal wiring lands with the first non-desktop consumer) and `DiscoveryProbe` (per-platform transport plug-point).
  - **`DiscoveryEngine` is now the desktop adapter**: DTO↔domain mapping at the network boundary; the registry owns state; the adapter keeps platform feeds (JmDNS/UDP beacons, identity re-advertisement, dual-port UDP + HTTPS/HTTP manual probe) unchanged. The legacy wire-typed `devices` flow is bridged with SYNCHRONOUS write-through — every existing consumer (UI, engines, routes) compiles and behaves identically; the full legacy DiscoveryEngineTest (disclosure rule, cache semantics, probe parsing) passes unmodified against the registry-backed adapter.

## [10.1.31.3] - 2026-09-01
### Added
- **[minor] Sync Client Loop Closed (plan 031 WP4 — plan now DONE)**:
  - **`DesktopSyncScheduler`**: the always-on flush loop — 5-minute periodic exchange that also PULLS host records when the queue is empty (fresh-boot convergence), idles safely when sync is disabled (no host configured) or signed out (queued deltas retained, never dropped), and backs off on failure (cadence-magnitude — an unreachable host never becomes a busy loop). Started with the server lifecycle, cancelled by the desktop shutdown coordinator (queued deltas persist to DataStore — offline-first).
  - **Settings-gated sync host**: `DeviceConfig.syncHostUrl` (persisted, empty = disabled, zero hardcoded endpoints); the transport resolves it per exchange so a settings change takes effect without restart (test-pinned).
  - **Live Google ID token for sync auth**: `GoogleOAuth` now retains the raw ID token in memory (process-lifetime only — never persisted, cleared on sign-out) and the transport presents it as the exchange bearer.

### Fixed
- **[major] SyncEngine convergence defect**: `flush` previously returned early on an empty delta queue, so a device with no local changes NEVER pulled host records — breaking the "new device instantly inherits history/settings" promise. Empty-delta flushes now still perform the exchange.
- **[minor] DeviceConfig init race**: the async DataStore hydration could clobber a `syncHostUrl` write that fired before initialization completed (any early settings write would silently revert). Init now never overwrites a setter-touched flow — the newest user intent always wins.

## [10.1.32.0] - 2026-09-01
### Added
- **[major] `server/` Module — Streaming E2EE Relay + Sync Host (plan 032 WP1)**:
  - **New JVM module `server/`** (Ktor 3.x Netty): the self-hosted cloud peer for the Hetzner VPS. Consumes ONLY `core/protocol` + `core/sync` (the two wire laws) — structurally incapable of growing desktop dependencies. Shadow fat JAR (`dex-server-all.jar`), non-root Dockerfile (streaming law = zero disk writes), GitHub Actions deploy workflow (all credentials via Actions secrets, env-only config: `DEX_PORT`, `DEX_GOOGLE_CLIENT_ID`).
  - **Google ID Token auth** (`GoogleIdTokenVerifier`, current documented approach — research-verified): signature/iss/aud/exp against Google's cached public keys; the tenant subtree is derived ONLY from the verified `sub` claim (the same `googleSub` namespace the trust model uses). The server REFUSES to boot without real verification (fixture verifier double-gated for dev only) — a silent open relay is the one failure mode that must never ship.
  - **Sync host surface** `/sync/v1/exchange`: the core/sync wire law verbatim — same `SyncExchangeRequest/Response` DTOs, same `supersedes` HLC-LWW law as every peer; in-memory tenant store (cross-tenant access structurally impossible); illegal collections dropped at the door. 6 contract tests: auth gates, tenant isolation, merge law, stale-resend immunity, illegal-collection rejection.
  - **NAT-punch rendezvous** `/punch/register` + `/punch/resolve`: 5-min TTLs (desktop parity), self-contained JSON responses.
  - **Streaming relay** (`RelaySessionRegistry` + REST surface): quotas BEFORE first byte (2 concurrent sessions/tenant, 2 GiB/session, 10-min idle + 1-h hard TTL — CX22 budget), bounded SharedFlow pass-through — content NEVER touches disk, bytes stay OPAQUE end-to-end (the relay structurally cannot read content; session keys live on peers), data plane gated by the unguessable per-session stream token. 11 contract tests: auth/token gates, quota rejection, opaque byte pass-through, session lifecycle, punch round-trips.
- **[minor] Root build hardening**: the `allprojects` test-router task now registers only for KMP/Android projects — the previous unconditional `tasks.register("test")` collided with the Java plugin's built-in `test` task and broke plugin application for every JVM module (the first one being `server/`).

### Changed
- **[fix] Android DeX UI Visibility & Native RenderNode Crash Fix**:
  - **LaunchedEffect Entrance Trigger**: In `NavBottomSheet.kt`, replaced the single-key `remember(totalHeightPx)` with a persistent `Animatable(0f)` and a `LaunchedEffect(totalHeightPx, initialTier)` entrance animation. This prevents the bottom sheet from getting stuck at height 0 (100% collapsed translation off-screen) due to missing `Lifecycle.Event.ON_RESUME` on cold launch.
  - **Backdrop RenderNode Recursion Fix**: Removed self-referential `.layerBackdrop(sheetContentBackdrop)` modifier from `MorphingSheetNavPill` and eliminated the duplicated `MorphingSheetNavPill` composable invocation in `Navigation.kt`. This eliminates cyclic backdrop sampling in `libhwui.so`, resolving a fatal native `SIGSEGV` (stack overflow in `RenderNode::prepareTreeImpl`) on Vulkan-rendered devices.
- **[minor] Plan 032 status**: WP1 DONE (module + all three surfaces + deploy scaffolding, 17 tests green); follow-ups are client-side WAN wiring, E2EE session-key plumbing, load testing, and first deploy (user decisions: VPS provisioning, domain/DNS).

## [10.1.31.2] - 2026-09-01
### Added
- **[minor] Sync Transport + Feature Wiring (plan 031 WP3)**:
  - **Sync REST wire law (`core/sync`)**: `SyncExchangeRequest`/`SyncExchangeResponse` DTOs + `SyncEndpoints` paths defined once in the pure sync module — plan 032's server consumes these exact definitions, making client↔host exchange drift structurally impossible. Auth is the live Google ID token; the tenant is the verified `googleSub` (the same identity namespace the trust model already uses — never a Firebase UID).
  - **`HttpSyncTransport` (core/network)**: authenticated Ktor POST exchange with failure propagation to the engine (offline-first: a failed exchange re-queues, never loses data). 4 embedded-server tests: full DTO round-trip, HTTP error propagation with status surfaced, blank-token refusal ("sign in first"), empty-delta legality.
  - **`SyncBridge` (core/data)**: the single choke point from desktop domain state into the sync engine — HISTORY records (metadata only by construction of the model), the own-device DEVICES roster card (alias/deviceModel/platform, published at server start), and DEVICES tombstones on identity reset. Koin-optional: absent engine means local-only operation, never a startup failure.
  - **Wiring**: `TransferHistory.log` returns its record (recorder queues sync mutations without re-reading storage); `TransferHistoryRecorder` bridges every history write through the bridge; `TrustRevocationService.revokeAll` tombstones the old fingerprint before rotating identity; `DeXServer.start` attaches the engine and publishes the roster card, failure-tolerant.
- **[minor] Plan 032 amended**: the future `server/` module consumes `core/sync` for the sync wire law in addition to `core/protocol`.

## [10.1.31.1] - 2026-09-01
### Added
- **[minor] Sync Persistence Layer (plan 031 WP2)**:
  - **`DataStoreSyncStorage` (core/data)** — the desktop `SyncStorage` adapter: one serialized `SyncRecord` per `(collection, key)` under `sync.*` preference keys plus the persisted HLC state; corrupt entries and clock values degrade to absent (never block startup or crash a flush — the repo-wide persistence tolerance rule). 8-test contract suite: payload + tombstone round-trips, per-collection snapshots, purge, clock persistence across adapter restarts, corrupt-entry tolerance.
  - **DI wiring (NetworkModule)** — `SyncStorage` over the app's DataStore, `HybridLogicalClock` on the platform wall clock with persisted state restored (monotonic-only) at graph construction, `SyncEngine` with `deviceId = DeviceConfig.fingerprint`.
  - **Layering hardened**: `core/sync` is now a pure leaf (protocol + coroutines + serialization; the `HashUtils` default clocks were removed — HLC and engine take mandatory wall-clock injection), preventing the `sync ↔ data` dependency cycle. Final graph: sync (pure) ← data (adapter) ← network (composition).

## [10.1.30.2] - 2026-09-01
### Added
- **[major] core/domain Layer — Transfer Registry Slice (plan 027)**:
  - **`TransferUseCase` & `TransferModels`**: Platform-neutral transfer session registry — `TransferSession` (verbatim legacy `TransferStateMonitor` shape), `TransferFile`, `TransferOffer`, `TransferProgress`, `TransferOutcome`; register / reportProgress / markComplete / completeSession (with 6s linger) / removeSession; history direction (`received`/`sent`) and status (`success`/`failed`) constants frozen by test.
  - **Pinned legacy contract**: `markComplete` never auto-removes — callers own entry lifetimes (desktop dashboard + overlay tests depend on it); progress reports replace fields verbatim; blank session ids never register.
  - **Facade + single history funnel**: `core/network`'s `TransferStateMonitor` now delegates to the domain use case (public API byte-identical; one process-wide instance via Koin with partial-DI-safe fallback); `TransferHistoryRecorder` is the only writer of `TransferHistory.log` across ShareRoutes (upload success/failure), DesktopPullService (pull success/failure), and DesktopFileSendService (sent success/failed).
  - **Contract suite (`TransferUseCaseTest`)**: 9 tests — register/replace semantics, unknown-session no-op, linger timing, no-auto-removal pin, constant freezes.
- **[major] core/sync Layer — Offline-First Metadata Sync Engine (plan 031, Work Package 1)**:
  - **New module `core/sync`**: offline-first, metadata-only sync between DeX peers via the self-hosted sync host (decision settled 2026-08-31: Hetzner VPS, not Firebase). Depends only on `core/protocol`, `core/data` primitives, coroutines, serialization.
  - **`HybridLogicalClock`**: Kulkarni-style `physical.counter` timestamps — strictly monotonic under frozen or backwards wall clocks; causally absorbs remote stamps (`receive` beats both sides); monotonic-only `restore`. No drift bounds or rollover machinery — by design, the algorithm needs neither.
  - **`SyncRecord` & `SyncEngine`**: deterministic HLC-LWW convergence (`supersedes()`: HLC order, deviceId tiebreak); offline-first local writes persist immediately and queue coalesced deltas; `flush` exchange is idempotent and re-queues on failure; tombstones block stale-peer resurrection and compact after 30 days for own records only.
  - **Strict Privacy Law Enforcement**: `SyncCollections` locks the only legal surfaces (devices/history/settings); `FORBIDDEN_PAYLOAD_FIELDS` reject file/clipboard content and token values at any nesting depth before anything enters the queue.
  - **`SyncPorts`**: `SyncStorage`, `SyncTransport`, `SyncChangeSource`, `SyncScheduler` — DataStore/Ktor adapters stay platform-side (server surface is plan 032).
  - **Contract suite (`HybridLogicalClockTest`, `SyncEngineTest`)**: 23 tests — frozen/backwards-clock monotonicity, the skewed-phone causal-supersession scenario, receive-time absorption, tombstone resurrection immunity, offline re-queuing, coalescing, privacy rejection, compaction scoping.
- **[minor] `MessageHandler` scope hygiene (real defect fix)**: pairing/trust persistence (`pair-accepted`, `unpair`, `trust-check` downgrade) previously ran on unowned `CoroutineScope(Dispatchers.IO)` fire-and-forget coroutines that outlived test teardown and poisoned the next test class (`UncaughtExceptionsBeforeTest`). The handler now owns a supervised injectable scope; tests call `shutdown()` in teardown.
...
- **[minor] Android DeX: Protocol Constants Centralization & Dead Code Removal (Refactor Phase 1/4)**:
  - **Centralized Wire Protocol Registry (`network/ProtocolKeys.kt`)**: Introduced `ProtocolKeys` holding every WebSocket message type (~28 literals previously scattered across 9 files), envelope field names (`type`/`data` law per `docs/PROTOCOL.md`), and a canonical `envelopeOf()` builder so the outbound `{type, data}` shape can never drift per call site. All values moved verbatim — zero wire-format change. `ProtocolMessages` carries the three frame-constant messages (`pair-request`, `unpair`, `device-roster`) previously written as raw string literals in `WebSocketClientService`.
  - **Canonical API Routes (`network/NetConfig.kt` → `ApiRoutes`)**: Consolidated the 7 duplicated URL-path literals (`/api/localsend/v2/prepare-upload`, `/upload`, `/api/dex/clipboard`, `/download`, `/punch/endpoint`, `/api/localsend/v2/info`, `/ws`) previously repeated across `ClientEngine`, `QuicClient`, `BatchDownloadWorker`, and `PunchSession`, with `httpsUrl`/`httpUrl` builders preserving byte-identical URLs.
  - **Single Shared Json Instance (`DexJson`)**: Replaced the three separate `Json { ignoreUnknownKeys = true }` instances in `PunchSession`, `FileShareManager`, and `MessageHandler` with one shared `DexJson` (same configuration). `TransferHistory`'s stricter instance (coerceInputValues + encodeDefaults) deliberately kept separate — its flags change decode behavior. Bare `Json` decodes in workers now use `DexJson` too.
  - **WorkManager Input-Key Vocabulary (`TransferWorkKeys`)**: Centralized the `ip`/`port`/`uris`/`targetFingerprint`/`targetAlias`/`targetIdentityHash`/`targetGoogleSub`/`folderTreeUri`/`files`/`httpsPort` key strings that were defined independently at 5 writer sites (`ShareTargetActivity`, `Navigation`, `MainScreen`, `AppFunctions`, `PendingShareForwarder`) and 3 reader sites (`UploadWorker`, `BatchDownloadWorker`, `PunchSendWorker`), eliminating the silent key-drift hazard between enqueuers and workers. All string values unchanged.
  - **Notification Intent Contract (`TransferIntents`)**: The `ACCEPT_TRANSFER`/`REJECT_TRANSFER` broadcast actions and `SESSION_ID`/`NOTIFICATION_ID`/`FINGERPRINT` extras previously shared by convention between `NotificationHelper` and `FileTransferReceiver` are now defined once and referenced by both.
  - **Network Tuning Constants (`NetConfig`)**: Named the previously-anonymous operational values — `[SKIP]` token sentinel (was 4 scattered literals), 200ms UI throttle, concurrency cap 3, retry caps, 20s discovery TTL, 100-device DoS cap, UDP presence intervals (3s/10s/30s burst), multicast group `224.0.0.167`, NSD type `_dex._udp`/name `DeX_Android`, pairing timeouts. Numbers unchanged; only named and referenced.
  - **PIN Length Protocol Constant (`ProtocolKeys.PIN_LENGTH = 5`)**: The magic `5` hardcoded at 4 sites in the pairing UI (`ErrorDialogs.kt` `repeat(5)`, `length <= 5`, `length == 5`, `PinInputField`) now references the protocol-defined digit count in one place.
  - **Dead Code Removal**: Deleted `HistoryScreen.getDateGroupLabel` (verified unused — grouping is computed inline with a different algorithm); deleted dead `PROMPT_TIMEOUT_MS`/`GRANT_WAIT_MS` constants in `MessageHandler` (unused / duplicated the live one in `FileShareManager`); removed 9 unused imports from `MainScreen.kt` and 7 from `DiscoveryComponents.kt`; removed a duplicated `graphicsLayer` import from `ErrorDialogs.kt`.
  - **Scope**: Android app only (`DeX/app`). Desktop Compose modules and `Archived_Legacy_WPF/` untouched. Part of the phased refactor plan 024 (`advisor-plans/024-android-refactor-centralization.md`); verified with `:app:assembleDebug` green.
