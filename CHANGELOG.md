...
## [Unreleased]
### Fixed
- **[fix] Wire protocol compliance & envelope parsing hygiene (Audit Remediation)**: Added `STREAM_TOKEN`, `RELAY_URL`, `FILE_NAME`, and `SIZE` to `FieldNames` in `:core:protocol` and forwarded them in `DeX/app`'s `ProtocolKeys` shim; added golden fixture test `relay_offer_envelope_is_canonical`; eliminated hand-rolled envelope decoding in `MessageHandler.kt` and `WebSocketRoutes.kt` via `ProtocolEnvelope.decodeType`/`decodeData`; replaced raw string literals (`"sessionId"`, `"streamToken"`, `"relayUrl"`, `"fileName"`, `"size"`, `"fingerprint"`) across `DesktopFileSendService`, `WanRelayClient`, `DiscoveryEngine`, `DesktopUdpService`, and `:server`'s `RelayRoutes` with `FieldNames.*`; and updated `docs/ARCHITECTURE.md` to document the KMP `androidMain` targets.
