...
- Phase 2 DONE (2026-09-02): SettingsSwitchRow shadow consolidated in SettingsComponents; centralized ui/util/Formatters (formatBytes, formatDuration, formatSpeed) adopted across HistoryScreen, MediaPickerTray, ShareSheetContent with unit tests; UploadWorkRequestFactory centralized across AppFunctions, Navigation, ShareTargetActivity, PendingShareForwarder with unit tests; LiquidGlassExperiment moved to src/debug; `:app:assembleDebug` and `:app:testDebugUnitTest` green.
- Phase 3 DONE (2026-09-02): Fixed HistoryScreen:231 LIST->LIST toggle bug; eliminated DeviceManager lateinit prefs crash hazard with lazy initialization; extracted TransferProgressNotifier delegate adopted across UploadWorker and WanDownloadWorker with unit tests; extracted PairingCoordinator from MessageHandler decoupling pairing, countdown, and identity challenge logic with unit tests; `:app:assembleDebug` and full test suites green.
- Phase 4 deferred (own plan entries).
