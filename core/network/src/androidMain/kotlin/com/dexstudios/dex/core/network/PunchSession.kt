...
class PunchSession(
    ...
    private val wsService: WebSocketEngine,
    private val notificationHelper: INotificationHelper,
    private val downloadStateUpdater: IDownloadStateUpdater,
    ...
    private suspend fun registerEndpoint() = withContext(Dispatchers.IO) {
        ...
        try {
            val pcIp = wsService.connectedIp ?: PcMemory.ip() ?: return@withContext
    ...
    private suspend fun handleIncoming(socket: Socket) = withContext(Dispatchers.IO) {
        try {
            ...
            for (file in manifest.files) {
                ...
                val docUri = existing?.docUri?.let { android.net.Uri.parse(it) } ?: (if (dirUri != null) {
                    if (!relPath.isNullOrBlank()) {
                        SafStorage.createDocumentWithPath(context, dirUri, relPath)
                    } else {
                        SafStorage.createDocumentUri(context, dirUri, file.fileName)
                    }
                } else {
                    SafStorage.createMediaStoreUri(context, file.fileName, relPath)
                } as android.net.Uri?) ?: continue
                val out = context.contentResolver.openOutputStream(docUri, "wa")
                    ?: continue
                if (existing == null) resumeMap[file.id] = ResumeEntry(docUri.toString(), 0L, file.size)
                ...
                try {
                    ...
                    doneFiles++
                    TransferHistory.log(file.fileName, received, "received", docUri.toString())
                    downloadStateUpdater.updateState(
                        DownloadState(
            ...
            downloadStateUpdater.updateState(DownloadState(fileName = "$doneFiles of ${manifest.files.size} files", progress = 1f, isSuccess = true, doneFiles = doneFiles, totalFiles = manifest.files.size))
    ...
    private suspend fun runTransfer(
        ...
        for ((index, file) in files.withIndex()) {
            ...
            TransferHistory.log(file.fileName, file.size, "sent", uris[index].toString())
        }
