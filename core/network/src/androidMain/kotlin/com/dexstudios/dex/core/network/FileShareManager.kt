...
class FileShareManager(
    ...
    private val pullFg: IPullForegroundService,
    private val context: Context,
) : KoinComponent {
    private val wsService: WebSocketEngine by inject()
    ...
    private suspend fun replyPull(requestId: String, data: JsonObject) = withContext(Dispatchers.IO) {
        ...
        val pcIp = wsService.connectedIp ?: PcMemory.ip()
        if (pcIp == null) {
        ...
        val prepareRequest = PrepareUploadRequestDto(
            info = RegisterDto(
                alias = getPlatformDeviceName(), version = "2.0", deviceModel = android.os.Build.MODEL ?: "Android",
        ...
        pullFg.start(context, requestId, fileMeta.size)
                ...
                launch(Dispatchers.IO) {
                    ...
                    try {
                        ...
                        if (fileToken == null || fileToken == "[SKIP]") {
                            ...
                            TransferHistory.log(m.name, m.size, "sent", m.uri?.toString())
