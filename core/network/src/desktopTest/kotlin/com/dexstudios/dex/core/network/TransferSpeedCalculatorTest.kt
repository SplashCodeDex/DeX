...
class TransferSpeedCalculatorTest {
    ...
    fun testInstantaneousAndSmoothedSpeedCalculation() {
        ...
        // After 1000ms, 10MB transferred -> 10MB/s (10,000,000 B/s)
        val s2 = calc.sample(currentBytes = 10_000_000L, totalBytes = 100_000_000L, nowMs = 2000L)
        assertEquals(10_000_000L, s2.speedBps)
        assertEquals(9L, s2.etaSeconds) // 90MB remaining / 10MB/s = 9 seconds

        // After another 1000ms, 5MB transferred -> instant is 5,000,000 B/s
        // EMA: (10,000,000 * 7 + 5,000,000 * 3) / 10 = 8,500,000 B/s
        ...
        assertEquals(10L, s3.etaSeconds) // 85MB remaining / 8.5MB/s = 10 seconds
    }
