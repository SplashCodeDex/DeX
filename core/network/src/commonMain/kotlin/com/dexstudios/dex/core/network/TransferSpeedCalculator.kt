...
/**
 * Thread-safe Exponential Moving Average (EMA) transfer speed and ETA calculator.
 *
...
class TransferSpeedCalculator {
    ...
    data class SampleResult(val speedBps: Long, val etaSeconds: Long?, val formattedSpeed: String, val formattedEta: String)
     ...
     * @param nowMs Current epoch time in milliseconds (defaults to [HashUtils.currentTimeMillis]).
     */
    fun sample(currentBytes: Long, totalBytes: Long = 0L, nowMs: Long = HashUtils.currentTimeMillis()): SampleResult = synchronized(lock) {
    ...
    companion object {
        ...
        fun formatSpeed(speedBps: Long): String {
            ...
            return when {
                speedBps >= gb -> {
                    val v = kotlin.math.round((speedBps / gb) * 10.0) / 10.0
                    "$v GB/s"
                }

                speedBps >= mb -> {
                    val v = kotlin.math.round((speedBps / mb) * 10.0) / 10.0
                    "$v MB/s"
                }
