 ...
 * When touched, the component scales down (squishes) or outward (swells).
 * When swiped/dragged, the component slightly pulls towards the pointer.
 * When released, it bounces back with natural jelly elasticity.
 *
 * @param targetScale The scale to squish down to or swell out to on press.
 * @param pullFactor How much the bubble pulls towards the pointer (0.0 to 1.0).
 * @param elasticity Degree of jelly elasticity (0.0f = rigid to 1.0f = hyper-jelly wobble).
 * @param scalePressSpeed Stiffness speed for press parallax scaling (default: 288f).
 * @param scalePressDamping Damping ratio for press parallax scaling smoothness (default: 0.35f).
 * @param scaleSettleSpeed Stiffness speed for press scale release-to-settle spring back to 1.0f (default: 300f).
 * @param onPhysicsUpdated Callback receiving live physics values (scale, translationX, translationY).
 * @param onPressedChanged Callback notified when physical touch press state changes.
 */
fun Modifier.bubbleFluidity(
        ...
        SuspendingPointerInputModifierNode {
            awaitEachGesture {
                ...
                scaleJob = coroutineScope.launch {
                    ...
                    if (targetScale >= 1.0f) {
                        // Swell / Expansion (e.g. 1.0 to 1.08):
                ...
                try {
                    ...
                    while (pointerEvent.changes.any { it.pressed }) {
                        ...
                        // Jelly Elasticity: When pointer drags/rubs, it stretches along the direction of motion
