...
// RelayCrypto JVM actuals (plan 032): javax.crypto only — AES-256-GCM AEAD + HKDF via
// the standard extract-then-expand construction over HMAC-SHA256 (RFC 5869).
...
actual fun actualDeriveRelaySessionKey(ikm: String, salt: String, info: String): ByteArray {
    ...
    // (a single block — no iteration needed for a 256-bit key).
...
actual fun actualRelayOpen(key: ByteArray, sealed: ByteArray): ByteArray {
    ...
    return try {
        ...
        // AEAD failure covers tamper, truncation, and wrong key uniformly — fail closed.
        throw RelayCryptoException("chunk failed authentication: ${e.javaClass.simpleName}")
    }
}

// Frame-sequenced AEAD: the 8-byte big-endian sequence rides the GCM AAD so replay,
// reorder, or gap detection is cryptographic, not heuristic.
...
actual fun actualRelayOpenFrame(key: ByteArray, seq: Long, sealed: ByteArray): ByteArray {
    ...
    return try {
        ...
        // Wrong sequence, replayed frame, reorder, tamper, or wrong key: all fail closed.
